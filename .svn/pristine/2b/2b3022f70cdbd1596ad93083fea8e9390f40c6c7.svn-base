...
use Google\Client;
$branchname='';
        $path = '..\util\Dayclose_sms_details';
        if (file_exists($path)) {
...
$sql_branch =  $database->mysqlQuery("Select be_decimal,be_sms_on_dayclose,be_branchname,be_address,be_email,be_phone,be_others1,be_others2,be_others3,be_footer1,be_footer2,be_footer3,be_footer4 from tbl_branchmaster where be_branchid='1'"); 
				...
				while($result_branch  = $database->mysqlFetchArray($sql_branch)) 
					{
						...
						$_SESSION['s_sms_on_dayclose']=$result_branch['be_sms_on_dayclose'];
...
if($_SESSION['s_sms_on_dayclose']=="Y") // sms yes or no
{
    ...
    if($_REQUEST['type']=="summary")
	{
        
          $sms_list=$_REQUEST['list'];       
        $string="";
        $strings="";
        $stringtacshd="";
        $stringstacshd="";
...
//                                          
    ...
    $resu1=explode("Dlr Status: ",$response12['messages']);
                ...
                $sql_smssent_updation  =  $database->mysqlQuery("Update tbl_dayclose set dc_dayclose_sms_success='Y',dc_last_sms_time=NOW() where dc_day='".$_SESSION['date']."'");
                }
		fpassthru($fp);
		fclose($fp);
          ...
          if(!file_exists("..\util\Dayclose_sms_details/".$_SESSION['date'].".txt")) 
         {
          $myfile = fopen("..\util\Dayclose_sms_details/".$_SESSION['date'].".txt", "w") or die("Unable to open file!");
        ...
        $sql_login_fire  =  $database->mysqlQuery("select tf_active from tbl_firebase_notification_report where tf_report_head='Day Close' "); 
...
if($_SESSION['cloud_enable_sync']=='Y' && $_SESSION['firebase_on']=="Y" && $firebase_report_status=="Y" ) // sms yes or no
{
    $a="8.8.8.8";
...
//                      }
		...
		$date_data='';
                $date_data=explode('-',$data);
                 $smstext=" DAY CLOSE  \n";
            $smstext=" Sales Summary  \n";
            if($sms_show_onoff[2]=='Y'){
                if($total_exclusive > 0){
                   $smstext.= "\nTotal Sale (Excl Tax)  :".number_format($total_exclusive,$_SESSION['be_decimal']);
                }
            }
            if($sms_show_onoff[1]=='Y'){
                if($finaltotal > 0){
                   $smstext.= "\nTotal Sale (Incl Tax) -".number_format($finaltotal,$_SESSION['be_decimal']);
                }
            }
            if($sms_show_onoff[3]=='Y'){
                if($kotsum > 0){
                   $smstext.= "\n KOT Sale -".number_format($kotsum,$_SESSION['be_decimal']);
                }
            }
            if($sms_show_onoff[7]=='Y'){
                if($totalcashdi > 0||$subtotalcredit > 0||$subtotalvoucher > 0||$subtotalcoupon > 0||$subtotalcoupon > 0||$subtotalcheque > 0||$subtotalcp > 0||$subtotalcomp > 0){
   ...
   $projectId = 'ed-reports-b5f94'; 
    ...
    'message' => [
       "topic"=> $branch_id_fire,
        'notification' => [
            'title' => $_SESSION['s_branchname']."  - SALE CLOSED ",
            'body' => $body
        ],
        'data' => [
            'key1' => 'value1',
...
//    curl_close($ch);
    ...
    $data_to_firebase=urlencode($smstext);
