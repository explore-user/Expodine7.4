            ...
            <div class="col-md-12">                      
                      <div style="margin-bottom:0;background: #fff;" class="cc_new">
                       	<div style="border: 0 !important " id="lista1" class="als-container">
                               <h3 style="float: left;margin-top: 10px;margin-left: 10px;">CONTRA VOUCHER</h3>
                           <div style="width: 140px;float: right;top: 3px;height: 33px;margin: 8px 10px;" class="search_btn_member_invoice filte_new_box_btn">
                           <a class="md-trigger"  style="background-color:#314b6b;margin:0;line-height: 32px;" onclick="return pop_on();" href="#">ADD CONTRA VOUCHER</a></div>                        
                      </div>
                    </div>
           </div>
					...
					if($num_kotlist){
                        ...
                        <?php 
                             ...
                             <div class="acc_add_box" style="width:20%;">
                                    ...
                                    <a class="form-control filte_new_box " style="background-color: #890000;color: white ;width: 63px" href="contra_voucher.php" >REFRESH </a>      
                                </div>
                                    ...
                                    <table class="acc_table_scroll">
                                        <thead>
                                            <tr>
                                                ...
                                                <td style="min-width:125px">From Acc</td>
                                                <td style="min-width:125px">To Acc</td>                                            
                                                <td style="min-width:175px;">Trn Details</td>
                                                <td style="min-width:100px">Entry Date</td>
                                                <td style="min-width:250px">Particulars</td>                                           
                                            </tr>
                                        </thead>
...
<div  class="md-modal md-effect-16 printer_add_popup" id="modal-17" style="top:0;width:100%;height:100%">
			<div style="width:800px;top:3%;margin:auto;left:0;right:0" class="md-content">
                            <h3 id="head_pop" style="margin-bottom:0"> ADD CONTRA VOUCHER </h3>
                ...
                <div class="div">
                    ...
                    <div class="col-md-4">
                        <div class="group" id="prn_div">                            
                            <select class="add_printer_drop" id="cv_from" onchange="from_change();"> 
                             <option value="">Select</option>
                             <?php 
                                         $sql_kotlist  =  $database->mysqlQuery("SELECT tlm_id,tlm_ledger_name from tbl_ledger_master tl left join tbl_ledger_group tg on tl.tlm_group=tg.tlg_id where (tg.tlg_name='Cash in Hand' OR tg.tlg_name='Bank Accounts')  "); 
					$num_kotlist  = $database->mysqlNumRows($sql_kotlist);
                    ...
                    <div class="col-md-4">
                        <div class="group to_select" id="prn_div">                         
                            <select class="add_printer_drop" id="to_dummy">
                                <option value="">Select</option>
                            </select>
...
function pop_on()
{
    ...
    $('#cv_amount').val('');
    $('#cv_remarks').val('');
    $('#head_pop').text('ADD CONTRA VOUCHER');
    ...
    $.ajax({
        ...
        $('#to_dummy').hide();
       var from=$('#cv_from').val();
       var data_v="set=load_to_contra&from="+from;
  ...
  var cv_remarks=$('#cv_remarks').val();
    if(cv_amount!='' && cv_date!='' && cv_from!='' && cv_to!='' && cv_remarks!=''  ){
        var data_v="set=add_contra_voucher&cv_date="+cv_date+"&cv_amount="+cv_amount+"&cv_from="+cv_from+"&cv_to="+cv_to+"&cv_trans="+cv_trans+
   ...
   function search_contra(n)
   {
       ...
       var data_v="set=search_contra_voucher&fromdt="+cv_date_search+"&todt="+cv_date_search_to+"&toacc="+toacc_search;
   ...
   function datechange()
   {
    ...
    function edit_contra_voucher(vid){
        ...
        $('#upd_btn').show();
    $('.printer_add_popup').addClass('md-show');
    $('#head_pop').text('UPDATE CONTRA VOUCHER');
    $.ajax({
			type: "POST",
			url: "load_accounts_data.php",
			data: "set=edit_contra_voucher&edit_id="+vid,
			success: function(msg)
			{    
                       var ed1=$.trim(msg);                      
                       var ed=ed1.split('*');    
  ...
  $('#cv_remarks').val(ed[5]);
  $('#to_dummy').hide();
       var data_v="set=load_to_contra1&from="+ed[1]+"&to="+ed[2];
        $.ajax({
			type: "POST",
			url: "load_accounts_data.php",
			data: data_v,
...
function update_contra_voucher(){
  ...
  var update_id=$('.cv_date').attr('voucher_update_id');
    if(cv_amount!='' && cv_date!='' && cv_from!='' && cv_to!='' && cv_remarks!='' ){
        var data_v="set=update_contra_voucher&cv_date="+cv_date+"&cv_amount="+cv_amount+"&cv_from="+cv_from+"&cv_to="+cv_to+"&cv_trans="+cv_trans+
          "&cv_remarks="+cv_remarks+"&update_id="+update_id;        
        $.ajax({
			type: "POST",
			url: "load_accounts_data.php",
			data: data_v,
