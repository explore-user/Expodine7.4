...
?>
    ...
    <div class="overlay">
            <div class="error-box">
                <h2>⚠ DB CONNECTION LOST</h2>
                <p style="font-weight: bold">Please check the server ip & config </p>
                 <p>Contact Support : +917994051951</p>
            </div>
        </div>
...
function show_portion_short_ful_details($id)
	...
	{//`tbl_kotmaster`(`kr_date`, `kr_kotno`, `kr_print`, `kr_firstprint`, `kr_lastprint`)
        ...
        function dynamic_sms_api($num,$msg)
	{
      ...
      $msg1=urlencode("Hi  $var1,Thanks  For  Visiting  $var2. Expodine");
            
      $data = file_get_contents("https://2factor.in/API/R1/?module=PROMO_SMS&apikey=a2d110ed-a994-11eb-80ea-0200cd936042&to=$num&from=EXPDNE&msg=$msg1");
