            ...
            <div class="col-md-12">                       
                      <div style="margin-bottom:0;background: #fff;" class="cc_new">
                       	<div style="border: 0 !important " id="lista1" class="als-container">
                               <h3 style="float: left;margin-top: 10px;margin-left: 10px;">RECEIPT VOUCHER</h3>
                           <div style="width: 140px;float: right;top: 3px;height: 33px;margin: 8px 10px;" class="search_btn_member_invoice filte_new_box_btn">
                           <a class="md-trigger" data-modal="modal-17" style="background-color:#314b6b;margin:0;line-height: 32px;" onclick="return pop_on();" href="#">ADD RECEIPT</a></div>
                      </div>
                    </div>
           </div>              
                     ...
                     <div class="col-md-12" style="padding:0 5px;">
                            ...
                            <?php $date= date('Y-m-d'); ?>
                               <div class="acc_add_box" style="width:20%;">
                                    ...
                                    <input autocomplete="off" type="text" class="form-control filte_new_box search_name"  value="<?php echo $date; ?>"  onchange="return datechange();" id="datepicker1"  name="" placeholder="Date">
                                </div>
                               
                                <div class="acc_add_box" style="width:20%;">
                                    <label style="margin: 0;">To Date</label>
                                    <input autocomplete="off" type="text" class="form-control filte_new_box" value="<?php echo $date; ?>" onchange="return datechange();" id="datepicker2"  name="" placeholder="Date">
                                </div>
                               <div class="acc_add_box" style="width:20%;">
                                      <label style="margin: 0;"> ACC TYPE</label>
                                 <select class="form-control filte_new_box"onchange="return search_contra();"  id="toacc_search"> 
                             <option value="">Select</option>
                             <option value="Investment"> INVESTEMNT </option>
                                 ...
                                 <div class="acc_add_box" style="width:20%;">
                                    ...
                                    <a class="form-control filte_new_box " style="background-color: #890000;color: white ;width: 63px" href="receipts.php" >REFRESH </a>      
                                </div>
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <td style="width:10%">Date</td>
                                                  <td style="width:10%">Acc Type</td>
                                                <td style="width:10%">From Acc</td>
                                                <td style="width:10%">To Acc</td>
                                                <td style="width:10%">Amount</td>
                                                <td style="width:10%">Trn Details</td>
...
<div  class="md-modal md-effect-16 printer_add_popup" id="modal-17" style="top:0;width:100%;height:100%">
			<div style="width:800px;top:3%;margin:auto;left:0;right:0" class="md-content">
                            <h3 id="head_pop" style="margin-bottom:0"> ADD RECEIPT VOUCHER </h3>
                ...
                <div class="div">
                    ...
                    <div class="col-md-4">
                        <div class="group" id="prn_div">                            
                            <select class="add_printer_drop" id="from_acc" onchange="check_ledger_balance();">
                                <option value="">Select</option>
                                <?php 
                    $sql_kotlist  =  $database->mysqlQuery("SELECT tlm_id,tlm_ledger_name from tbl_ledger_master tl left join tbl_ledger_group tg on tl.tlm_group=tg.tlg_id where (tg.tlg_name='Loans and Advances' OR tg.tlg_name='Loan (Liability)' or tg.tlg_name='Investments' or tg.tlg_name='Indirect Income')  "); 
					...
					if($num_kotlist){
						while($result_kotlist  = $database->mysqlFetchArray($sql_kotlist)) 
                            ...
                            <label style="top:-5px;color: #414141;font-size: 14px" id="lab_grp">ACC Name</label>
                        </div>
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        <div class="group to_select" id="prn_div">   
                            <select class="add_printer_drop" id="acc_type">
                                <option value="">Select</option>
                                <option value="Investment"> INVESTEMNT </option>
                                <option value="Loan"> LOAN</option>
                                <option value="Indirect_income">INDIRECT INCOME</option>
                    ...
                    <div class="col-md-4">
                        <div class="group" id="prn_div">   
                          ...
                          <span class="highlight"></span>
                          <span class="bar"></span>
                          <label class="amount_div_show" id="lab_grp">Amount</label>
                        </div>
                    </div>
                    <div class="col-md-4">
                    <div class="group" id="prn_div">   
                    ...
                    <a id='add_btn' onclick="return add_receipt_voucher();" href="#"><button style="position:relative;top:2px;float:right;right:5px;height: 42px;" class="md-save">SUBMIT</button></a>
...
function pop_on()
{
    ...
    $("#received").val('');
    $("#remarks").val('');
    $('#head_pop').text('ADD RECEIPT VOUCHER');
    $('#add_btn').show();
    $('#upd_btn').hide();
}
...
function add_receipt_voucher(){
  ...
  var cv_receive=$('#received').val();              
    if(cv_amount!='' && cv_date!='' && cv_from!='' && cv_to!='' && cv_remarks!='' && cv_type!=''  ){
        var data_v="set=add_receipt_voucher&cv_date="+cv_date+"&cv_amount="+cv_amount+"&cv_from="+cv_from+"&cv_to="+cv_to+"&cv_trans="+cv_trans+
          "&cv_remarks="+cv_remarks+"&cv_type="+cv_type+"&cv_receive="+cv_receive;
        $.ajax({
			type: "POST",
			url: "load_accounts_data.php",
			data: data_v,
   ...
   function search_contra(n){
       var cv_date_search=$('#datepicker1').val();
       var cv_date_search_to=$('#datepicker2').val();
       var toacc_search=$('#toacc_search').val();
       var data_v="set=search_receipt_voucher&fromdt="+cv_date_search+"&todt="+cv_date_search_to+"&toacc="+toacc_search;
    
    // if($('.search_name').val()!='' && $('#datepicker2').val()!='')
    // {
   ...
   function datechange()
   {
    ...
    function edit_contra_voucher(vid){    
        $('#add_btn').hide();
        $('#upd_btn').show();
    $('.printer_add_popup').addClass('md-show');
    $('#head_pop').text('UPDATE RECEIPT VOUCHER');
    $.ajax({
			type: "POST",
			url: "load_accounts_data.php",
			data: "set=edit_receipt_voucher&edit_id="+vid,
			success: function(msg)
			{   
                       var ed1=$.trim(msg);                    
                       var ed=ed1.split('*');                 
...
function update_contra_voucher(){
 var cv_date=$('.cv_date').val();
  ...
  var update_id=$('.cv_date').attr('voucher_update_id');        
    if(cv_amount!='' && cv_date!='' && cv_from!='' && cv_to!='' && cv_remarks!='' && cv_type!='' ){    
        var data_v="set=update_receipt_voucher&cv_date="+cv_date+"&cv_amount="+cv_amount+"&cv_from="+cv_from+"&cv_to="+cv_to+"&cv_trans="+cv_trans+
          "&cv_remarks="+cv_remarks+"&update_id="+update_id+"&cv_type="+cv_type+"&cv_receive="+cv_receive;       
        $.ajax({
			type: "POST",
			url: "load_accounts_data.php",
			data: data_v,
