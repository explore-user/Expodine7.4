...
if(isset($_REQUEST['set_delete_rules_master'])&&($_REQUEST['set_delete_rules_master']=="delete_rules_master")){
    ...
    $database->mysqlQuery("DELETE FROM tbl_loyalty_pointrule WHERE lyp_id = '" .$mst_id_del."'");
...
if(isset($_REQUEST['set_update_master1'])&&($_REQUEST['set_update_master1']=="update_master1")){
             ...
             $amt1= $_REQUEST['amount1'];
             $mid1=$_REQUEST['up_id'];
           
               $sql_smsattemt_updation  =  $database->mysqlQuery("UPDATE tbl_loyalty_pointrule SET lyp_point='".$point1."',lyp_amount='".$amt1."' where lyp_id='".$mid1."'");
...
if(isset($_REQUEST['set_add_master'])&&($_REQUEST['set_add_master']=="add_master")){
        ...
        $insertion['lyp_point'] = mysqli_real_escape_string($database->DatabaseLink,trim($point)); 
        
        $insertion['lyp_amount']= mysqli_real_escape_string($database->DatabaseLink,trim($amt)); 
        
	$insertid      =  $database->insert('tbl_loyalty_pointrule',$insertion); 
...
if(isset($_REQUEST['set_edit_master']) && $_REQUEST['set_edit_master']=="edit_rules_master"){
  ...
  $loy_qry1_upd = $database->mysqlQuery("select * from tbl_loyalty_pointrule where lyp_id='".$_REQUEST['edit_master_id']."' ");
    $num_loy1_upd = $database->mysqlNumRows($loy_qry1_upd);
     if($num_loy1_upd)
     {
         while($loyalty_listing_edit = $database->mysqlFetchArray($loy_qry1_upd))
         {
             ...
             $pnt=  $loyalty_listing_edit['lyp_point'];
             $amt=  $loyalty_listing_edit['lyp_amount'];
...
<head>
    ...
    <body class="fixed-left">
            ...
            <div class="content-page">
                ...
                <div class="content">
                    ...
                    <div class="page_head_detl_sec">
                        <h2>   Point Rule  </h2>
                        <a data-toggle="modal" data-target="#earing-point-master-add" href="#"><div style="" class="top-head-add-btn"> +ADD</div></a>
                        <a href="redeem_rule.php"><div style="width:120px;background-color: darkred;margin-right: 10px;float:right" class="top-head-add-btn">Redeem Rule</div></a>
     ...
     $loy_qry_rules = $database->mysqlQuery("select * from tbl_loyalty_pointrule");
     ...
     if($num_loy_rules)
     {
         while($loyalty_listing_rules = $database->mysqlFetchArray($loy_qry_rules))
         {
                      ...
                      $mst_id=$loyalty_listing_rules['lyp_id'];
                                ?>
                                <tr>
                                    <td id="point_one" > <?=$loyalty_listing_rules['lyp_point']?></td> 
                                    
                                    <td><?=$loyalty_listing_rules['lyp_amount']?></td>  
                                    <td>
                                    	<a data-toggle="modal" data-target="#earing-point-master-add" onclick="return edit_master('<?=$loyalty_listing_rules['lyp_id']?>');"  href="#" class="action-btn" ><span class="ti-pencil"></span></a>
                                        <a href="#" class="action-btn" onclick="return delete_rules('<?=$loyalty_listing_rules['lyp_id']?>');" ><span class="ti-trash"></span></a>
									...
									<div class="modal-content inner-textbox-effect" style="display: inline-block;border: 0;padding: 0;width: 100%">
										<div class="col-md-6">
											<div class="group" style="margin-bottom: 0">
                                                                                            <input id="point_add" required="" name="" type="text">
												<label>Point</label>
										...
										<div class="col-md-6">
											<div class="group" style="margin-bottom: 0">
												<input id="amount_add" required="" name="" type="text" >
												<label>Amount</label>
												<span class="highlight"></span>
												<span class="bar"></span>
											 </div>
  ...
  function edit_master(em){
    ...
    function submit_rules_master(){
       ...
       if(sts!="update"){
          ...
          var point_one=$('#point_one').text();
      
        if(point_one==""){
                   var point=$('#point_add').val();
                   var amount=$('#amount_add').val();
                  
                  
                  
                  if(point!="" && amount!="" )   {       
         var data="set_add_master=add_master&point="+point+"&amount="+amount;
      ...
      alert("Point Rule Already Added");
      location.reload();
    }
    }else{
        
                   var point1=$('#point_add').val();
                   var amount1=$('#amount_add').val();
                  var up_id= $('#hidden_id').val();
                  
                  
                  if(point1!="" && amount1!="")   {       
         var data="set_update_master1=update_master1&point1="+point1+"&amount1="+amount1+"&up_id="+up_id;
