...
$_SESSION['bu_id']=$_REQUEST['menu'];
$sql_login  =  $database->mysqlQuery("select * from tbl_base_unit_master  where bu_id='".$_SESSION['bu_id']."'"); 
$num_cat_s  = $database->mysqlNumRows($sql_login);
if($num_cat_s){
  while($result_cat_s  = $database->mysqlFetchArray($sql_login)) 
	  {
			  $searchname=$result_cat_s['bu_name'];
	  }			
} 
else
{
...
//    }     
	 ...
	 ?>
                 <form role="form" action="base_unit_master.php"  method="post"  name="preference_masteredit">
                            <input type="hidden" name="baseunitid" id="baseunitid" class="menuname" style="color:black" value="<?=$result_login['bu_id']?>">       
                        	 <div class="first_form_contain">
                              <span id="prefrncstatus1234" class="load_error alertsmaster" style="color:#F00" ></span> 
                             	<div class="form_name_cc">Base Unit master<span style="color:#F00">*</span></div>
                                
                               	 <div class="form_textbox_cc" id="pref_div1">
                                <input type="text" class="form-control preferencenames" id="baseunit2" name="baseunit2"  placeholder="Base unit" tabindex="0"  data-toggle="tooltip" title="Preference" value="<?=$result_login['bu_name']?>" onchange="valiprefrnc('<?=$result_login['bu_id']?>')"></div>
                               </div>
<!--                                        <div class="first_form_contain">
...
function valiprefrnc(id)
{
	 $('#prefrncstatus1234').text('');
			var id1=id;
	 var a=$("#baseunit2").val().trim();
	if(a!="")
			{
	  $.ajax({
			type: "POST",
			url: "load_divcheckmenu.php",
			data: "value=unitadd1&mid="+a,
...
//prefrncstatus1234
	function update_preference()
			...
			{
				var id=$("#baseunitid").val();
				if($(".preferencenames").val()=="")
				{
					$("#pref_div1").addClass("has-error");
						  document.preference_masteredit.baseunit2.focus();
                                                  alert("Enter unit");
						  return false;
				}
                                
                           var alphanumers = /^[a-zA-Z0-9 ]+$/;
                              if(!alphanumers.test($("#baseunit2").val())){
                              $("#pref_div1").addClass("has-error");
                                document.preference_masteredit.baseunit2.focus();
