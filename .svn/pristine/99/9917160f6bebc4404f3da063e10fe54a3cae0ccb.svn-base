...
$database	= new Database();
   ...
   if(isset($_REQUEST['set'])&&($_REQUEST['set']=="cat1")){
    ...
    $querylang=$database->mysqlQuery("select l_pref_id from tbl_language_preference where l_pref_id='".$catid1."' and l_lang_id='". $_SESSION['idrr']."'");
    $num_gen9  = mysqli_num_rows($querylang);
		  if($num_gen9)
		  {
    
   
    $querylang1=$database->mysqlQuery("update tbl_language_preference set l_pref_name='".$cat1."' where l_pref_id='".$catid1."' and l_lang_id='".$_SESSION['idrr']."'");
        }else
    {
         $insertion['l_lang_id'] =  $_SESSION['idrr'];
         $insertion['l_pref_name'] = $cat1;
          $insertion['l_pref_id'] = $catid1;
          $insertid = $database->insert('tbl_language_preference', $insertion);
    }
                       	...
                       	<div id="lista1" class="als-container">
                            <div class="als-viewport" style="width:100% !important">
                                ...
                                <ul class="als-wrapper">
                                    ...
                                    <li class="als-item"><a href="preference_lang.php" class="new_tab_btn active_btn_1">Preference</a></li>
                                    ...
                                    <table class="responstable">  
                                        <thead>
                                         <tr>
                                         	...
                                         	<th>Preference-English</th>
                                                <th>Preference-<span id="newlang"><?=ucfirst($lang)?> </span></th>
                                        ...
                                        <tbody>
                                           ...
                                           <?php        
                                                     $sql_login1  =  $database->mysqlQuery("select c.pmr_id,c.pmr_name,
(select m.l_pref_name from tbl_language_preference m  where m.l_pref_id = c.pmr_id and m.l_lang_id ='".$_SESSION['idrr']."' )as name_lang  
from  tbl_preferencemaster c order by c.pmr_name"); 
                                               ...
                                               ?>   
                                                 <tr>
                                                     <td style="min-width:50px;">
                                                         <a href="#" style="display:block" class="edit_list text1" id="edtbtn<?=$result_login1['pmr_id']?>" onclick="edit_click('<?=$result_login1['pmr_id']?>')"  ><img src="images/edit_page.PNG"></a>
                                                         <a href="#" style="display:none" class="edit_list" id="savebtn<?=$result_login1['pmr_id']?>" onclick="save_click('<?=$result_login1['pmr_id']?>')"  ><img src="img/save_ico.png"></a>
                                                   </td>

                                                   <td ><?=$result_login1['pmr_name']?></td>
                                                   
                                               <td ><div style="display:block" id="catnameshow<?=$result_login1['pmr_id']?>"><?=$result_login1['name_lang']?></div>
                                                   <div id="catnameedit<?=$result_login1['pmr_id']?>" style="display:none"> <input type="text" style="text-align:left;padding-left:10px" class="language_textbx txt1" name="catnamefield" id="catnamefield<?=$result_login1['pmr_id']?>"  > </div></td>
                                                 </tr>
                                <?php } } ?>
