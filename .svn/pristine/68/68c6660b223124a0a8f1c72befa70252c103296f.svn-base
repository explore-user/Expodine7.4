    ...
    <body class="fixed-left">
        ...
        <div id="wrapper">
            ...
            <div class="" >
                 ...
                 <a style="font-size: 1.6rem;border-radius: 0.5rem;box-shadow: 0rem 0rem 0rem 0.1rem #fbaea4;color: #000000;background-image: linear-gradient( 223deg, #ffffff, #ffffff)!important; "  class="inv-req-btn1" href="#">PHYSICAL STOCK</a>
                <a class="inv-pro-btn1"  href="physical_stock_history.php">PSH</a>
                &nbsp;&nbsp;&nbsp;
                
                
                <a style="background-image: linear-gradient( 223deg , #45974c, #8ab796)!important;" class="inv-pro-btn1"  href="physical_default.php">DEFAULT</a>   
...
<h6 title="DOWNLOAD PHYSICAL STOCK ENTRY EXCEL SHEET" style="width: 3rem;"> <a  class="delete-all"  href="inventory_excel.php?set=download_physical_excel"><i class="fa fa-download fa-lg" aria-hidden="true"></i></a> </h6>
</div>
...
<div class="inv-req-form">
    ...
    <input style="width: 10rem;" id="weight" onkeypress="return numdot(this,event);"  placeholder="WEIGHT" type="text">
     ...
     <input style="width: 10rem;" id="qty" onkeypress="return numdot(this,event);"  placeholder="QTY" type="text">
...
<div class="inv-req-Submit">
  ...
  Items : <span id="rps_count">0</span>
    ...
    <?php 
  $store_on='';
   $fnct_menu1 = $database->mysqlQuery("select tps_store from tbl_physical_stock where tps_set='N' ");
         $num_fdtl1 = $database->mysqlNumRows($fnct_menu1);
        if ($num_fdtl1 > 0) {
              while ($result_fnctvenue1 = $database->mysqlFetchArray($fnct_menu1))
              {
                  $store_on=$result_fnctvenue1['tps_store'];
  ...
  <?php 
    ...
    <select id="type_physical1" style="display: none " >
        <option id="Normal">Physical Stock</option>
        <option id="Consumption">Consumption</option>
    </select>
...
$(document).on('keyup',function(e) {
 ...
 $('.calc').click( function(event) { 
		...
		$('#'+focused).focus();
          ...
          var datastring = "set=product_physical_load"
                 ...
                 { 
                   ...
                   $('#hidden_checker').val(sl);
                    $('#rps_count').text(sl); 
                    var product=record.tps_name;
                    var  qty=record.tps_qty;
                    
                    if(record.tps_brand=='null' || record.tps_brand=='' || record.tps_brand==null ){
                         var  brand= '';
                    }else{
                      brand=record.tps_brand;
                    }
                    
                    
                    if(record.tps_barcode=='null' || record.tps_barcode=='' || record.tps_barcode==null){
                         var  barcode= '';
                    }else{
                      barcode=record.tps_barcode;
                    }
                    
                     if(record.tps_weight=='null' || record.tps_weight=='' || record.tps_weight==null){
                         var  weight= '';
                    }else{
                      weight=record.tps_weight;
                    } 
                    
                     var  rate_type=record.tps_rate_type;
                     var  unit_type=record.tps_unittype;
              ...
              if($('.append_div_main').find('#del_card' + record.tps_id).length === 0) {
                  
              $(".append_div_main").append(" <div class='add_menu_row ' id='second_div_main"+record.tps_id+"' >"+
 "<div class='inv-req-form'>"+
    "<span class='inv-req-sl' style='width: 3%; overflow:hidden;display:none'>"+sl+"</span>"+
    
    "<input   id='product"+record.tps_id+"' value='" + product + "' readonly type='text' style='width: 12%;'>  "+
   
   " <input id='barcode"+record.tps_id+"' value='" + barcode + "' readonly  type='text' style='width: 12%;'>"+
   
    "<input value='" + rate_type + "' style='width: 12rem;' readonly id='rate_type"+record.tps_id+"' type='text'>"+
    
    "<input value='" + brand + "'  id='brand"+record.tps_id+"' readonly type='text' style='width: 12%;'>"+
    
   " <input value='" + unit_type + "'  style='width: 10rem;' id='unit_type"+record.tps_id+"' readonly type='text'>"+
   
    " <input value='" + weight + "'  "+chk_weight +"   onkeypress='return numdot(this,event);'    style='width: 10rem;' id='weight"+record.tps_id+"'  type='text'>"+
    
    " <input value='" + qty + "'   "+chk_qty +"   onkeypress='return numdot(this,event);'   style='width: 10rem;' id='qty"+record.tps_id+"'  type='text'><a title='Qty Update Button' style='margin-left:-45px' onclick='edit_por_qty("+record.tps_id+")'  ><i class='fa fa-check fa-lg' aria-hidden='true' style='color:green;cursor: pointer'></i></a>"+
    
    " <a class='inv-req-btn' onclick='delete_req("+record.tps_id+")'  style='width: 3rem;background-image: linear-gradient( 223deg,#cc1d35, #ff3a55)!important;color: #fff;cursor: pointer'>x</a>"+
   ...
   $("#plusbtn").click(function()
        {    
          ...
          var store= $("#store").val();
           ...
           var datastring2 = "set=check_product_phy&product="+product_id;
          ...
          if($.trim(data)=='yes'){
           ...
           var datastring = "set=add_product_physical&product="+product+"&barcode="+barcode+"&weight="+weight
                   +"&qty="+qty+"&brand="+brand+"&product_id="+product_id+"&unit_type="+unit_type+"&rate_type="+rate_type+"&edit_id="+edit_id+"&store="+store;
            ...
            if(product!='' && qty >= '0' && weight >='0' && store!='' ){
                 ...
                 $('#supplier').val(''); 
                     ...
                     $.each(a, function(i, record) {
                        ...
                        $("#second_div_main"+record.tps_id).empty() ;
                         $("#second_div_main"+record.tps_id).hide() ;
  ...
  "<div class='inv-req-form'>"+
   ...
   " <input value='" + weight + "'  "+chk_weight +"  onkeypress='return numdot(this,event);'   style='width: 10rem;' id='weight"+record.tps_id+"'  type='text'>"+
    ...
    " <a class='inv-req-btn' id='del_card"+record.tps_id+"' name='del_card"+record.tps_id+"' onclick='delete_req("+record.tps_id+")'  style='width: 3rem;background-image: linear-gradient( 223deg,#cc1d35, #ff3a55)!important;color: #fff;cursor: pointer'>x</a>"+
...
function confirm_yes_new(){
      ...
      var qty=  $('#qty'+id).val();
          ...
          var data="set=update_phy_qty&id="+id+"&qty="+qty+"&weight="+weight+"&unit="+unit+"&rate_type="+rate_type;
...
function numdot(item,evt) {
 ...
 function delete_req(id){
        ...
        var data="set=delete_physical&id="+id;
  ...
  function submit_req(){
    ...
    function submit_phy(){
        ...
        {     
         ...
         var data="set=add_physical_all&store="+store;
...
<div class="inv-Password-numbers" style="cursor:pointer">
  ...
  <a  class="inv-password-btn" href="#" onclick="submit_phy()">Login</a>
</div>
</div>
    </div></div>
