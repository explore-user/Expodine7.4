...
$database	= new Database();
   ...
   if(isset($_REQUEST['set'])&&($_REQUEST['set']=="cat1")){
    ...
    $querylang=$database->mysqlQuery("select fe_feedback_id from tbl_language_feedback where fe_feedback_id='".$catid1."' and fe_lang_id='". $_SESSION['idrr']."'");
    $num_gen9  = mysqli_num_rows($querylang);
		  if($num_gen9)
		  {
    
   
    $querylang1=$database->mysqlQuery("update tbl_language_feedback set fe_feedback_name='".$cat1."' where fe_feedback_id='".$catid1."' and fe_lang_id='".$_SESSION['idrr']."'");
        }else
    {
         $insertion['fe_lang_id'] =  $_SESSION['idrr'];
         $insertion['fe_feedback_name'] = $cat1;
          $insertion['fe_feedback_id'] = $catid1;
          $insertid = $database->insert('tbl_language_feedback', $insertion);
    }
                            ...
                            <div class="als-viewport" style="width:100% !important">
                                ...
                                <ul class="als-wrapper">
                                    ...
                                    <li class="als-item"><a href="preference_lang.php" class="new_tab_btn ">Preference</a></li>
                                      ...
                                      <li class="als-item"><a  id="mover" href="feedback_lang.php" class="new_tab_btn active_btn_1">Feedback</a></li>
                                    ...
                                    <table class="responstable">  
                                        <thead>
                                         <tr>
                                         	...
                                         	<th>Feedback-English</th>
                                                <th>Feedback-<span id="newlang"><?=ucfirst($lang)?> </span></th>
                                        ...
                                        <tbody>
                                           ...
                                           <?php        
                                                     $sql_login1  =  $database->mysqlQuery("select c.fbm_id,c.fbm_question,
(select m.fe_feedback_name from tbl_language_feedback m  where m.fe_feedback_id = c.fbm_id and m.fe_lang_id ='".$_SESSION['idrr']."' )as name_lang  
from  tbl_feedbackmaster c order by c.fbm_question"); 
                                               ...
                                               ?>   
                                                 <tr>
                                                     <td style="min-width:50px;">
                                                         <a href="#" style="display:block" class="edit_list text1" id="edtbtn<?=$result_login1['fbm_id']?>" onclick="edit_click('<?=$result_login1['fbm_id']?>')"  ><img src="images/edit_page.PNG"></a>
                                                         <a href="#" style="display:none" class="edit_list" id="savebtn<?=$result_login1['fbm_id']?>" onclick="save_click('<?=$result_login1['fbm_id']?>')"  ><img src="img/save_ico.png"></a>
                                                   </td>

                                                   <td ><?=$result_login1['fbm_question']?></td>
                                                   
                                               <td ><div style="display:block" id="catnameshow<?=$result_login1['fbm_id']?>"><?=$result_login1['name_lang']?></div>
                                                   <div id="catnameedit<?=$result_login1['fbm_id']?>" style="display:none"> <input type="text" style="text-align:left;padding-left:10px" class="language_textbx txt1" name="catnamefield" id="catnamefield<?=$result_login1['fbm_id']?>"  > </div></td>
                                                 </tr>
                                <?php } } ?>
