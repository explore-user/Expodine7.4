...
if($_SERVER['REQUEST_METHOD'] == 'POST' ) //&&  $_REQUEST['type']=="insert"
{
	...
	if($type=="insert ")
	{
	$insertion['sce_timein']=$finaldate;
	...
	}else if($type=="update ")
	{
		...
		$update['sce_timein']=$finaldate;
		
		mysqli_query($database->DatabaseLink,"Update tbl_staffattendence set sce_timein='".$finaldate."' where sce_staffid='".$_REQUEST['stw'] ."' and sce_date='".$_REQUEST['dt']."'");
...
/*else if($_SERVER['REQUEST_METHOD'] == 'POST' &&  $_REQUEST['type']=="update")
{
	$hour=$_REQUEST['hourset'];
	$minute=$_REQUEST['minset'];
	$ampm=$_REQUEST['ampm'];
          ...
          <div class="row">
              <div class="col-lg-12">
                  <form>
                      ...
                      <div style="width:100%; float:left;line-height:34px;font-size:14px;">TIME IN</div>
                        <?php
						...
						$sql_table_sel  =  $database->mysqlQuery("Select * from tbl_staffattendence Where sce_date = '".$date."' and sce_staffid='".$_REQUEST['stew']."' and sce_timein<>''"); 
					$num_table  = $database->mysqlNumRows($sql_table_sel);
					if($num_table){
						  while($result_table_sel  = $database->mysqlFetchArray($sql_table_sel)) 
							  { 
							  echo $result_table_sel['sce_timein'];
							  }
					}
						?>  
			...
			<form name="submitin" id="submitin" action="<?=$_SERVER['PHP_SELF']?>" method="post">
                ...
                <div style="width:100%; float:left;line-height:34px;font-size:14px;">TIME IN</div>
					...
					if($num_table)
					{
					  while($result_table_sel  = $database->mysqlFetchArray($sql_table_sel)) 
						  { 
						  		$in=$result_table_sel['sce_timein'];
