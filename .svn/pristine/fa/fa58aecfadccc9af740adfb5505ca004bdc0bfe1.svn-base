...
$_SESSION['pagid']=6;
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['unit1']))
{
	
	$insertion['u_name'] 		=  mysqli_real_escape_string($database->DatabaseLink,$_REQUEST['unit1']);

    $sql=$database->check_duplicate_entry('tbl_unit_master',$insertion);
	 if($sql!=1)
	{
	$insertid              			=  $database->insert('tbl_unit_master',$insertion);
	$database->updateexpodine_machines(); 
	 ...
	 if (!headers_sent())
    {    
        header('Location: unit_master.php?msg=2');
        exit;
        }
    else
        {  
        echo '<script type="text/javascript">';
        echo 'window.location.href="unit_master.php?msg=2";';
        echo '</script>';
        echo '<noscript>';
        echo '<meta http-equiv="refresh" content="0;url=unit_master.php?msg=2" />';
        echo '</noscript>'; exit;
    }
}
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['unit2']))
{
	$id=$_REQUEST['unitid'];
...
$query3=$database->mysqlQuery("update tbl_unit_master set u_name='$unit2' where u_id='$id'");
$database->updateexpodine_machines(); 

	 if (!headers_sent())
    {    
        header('Location: unit_master.php?msg=3');
        exit;
        }
    else
        {  
        echo '<script type="text/javascript">';
        echo 'window.location.href="unit_master.php?msg=3";';
        echo '</script>';
        echo '<noscript>';
        echo '<meta http-equiv="refresh" content="0;url=unit_master.php?msg=3" />';
        echo '</noscript>'; exit;
    }
}
$alert="";
...
<div class="mian">
	<div class="view-container">
		<div style=" top: 58px;"  id="container">
			<div class="breadcrumbs">
				<ul>
					...
					<li><a style="cursor:pointer">Unit Master</a></li>
				...
				<div class="als-viewport" style="width:100% !important">
                   ...
                   <div style="background:#fff; width:100%; height:auto; margin:0 0 5px 1px;padding-right:4px; border:1px #B6B6B6  solid;float:left">
                            ...
                            <div class="col-sm-2" style="padding-right: 5px;padding-left: 0px;margin-bottom:5px;width:25%">
                           		<span class="filte_new_text">Unit Master Name</span>
                                <input type="text" class="form-control filte_new_box" id="unit" name="unit" placeholder="Unit Name" onKeyUp="validateSearch()">
                            </div>
                            ...
                            <div class="col-sm-2 nopadding" style="width: 24.666667% !important;">
                                ...
                                <div style="margin-left:2%;width: 47%;" class="search_btn_member_invoice filte_new_box_btn"><a href="unit_master.php" >Reset</a></div>
                                  ...
                                  <div class="col-sm-2 nopadding" style="width: 24.666667% !important;">
                                      ...
                                      <div style="margin-left:2%;width: 47%;" class="search_btn_member_invoice filte_new_box_btn"><a style="    width: 154px;background-color: #683831" href="base_unit_master.php" >BASE UNIT MASTER</a></div>
                   ...
                   <div class="col-md-12 add_btn_cc_2">
                      <div class="btn_cc_2">
                   	<a tittle="Add" href="#"  id="add_unit" class="md-trigger add_btn_2" data-modal="modal-17" onClick="testpref()" ></a>
                      </div>  
                   </div>
                   <div class="col-md-12 contant_table_cc">
                        <table class="table_report scroll tablesorter"  width="100%" border="0" cellspacing="5" id="listall">
                            <thead>
                              <tr>
                                  <td width="9%">Sl No</td>
                                <th width="40%" height="35px">Unit</th>
         ...
         <?php
	  $sql_login  =  $database->mysqlQuery("select * from tbl_unit_master"); 
	 ...
	 ?>
    				<tr id="ids_<?=$result_login['u_id']?>"  class="select">
                                <td width="10%"><?=$i?></td>
                                <td><?=$result_login['u_name']?></td>
                                  
                                <td>
                                 <a href="#" class="md-trigger_prfrnc" id="ids_<?=$result_login['u_id']?>" ><img src="images/edit_page.PNG"></a>
                                <input type="hidden" name="hiddenmenuid" id="hiddenmenuid" value="<?=$result_login['u_id']?>">
			...
			<div class="md-content">
				...
				<div>
                    <div class="col-lg-12 col-md-12 no-padding">
                        <form role="form" action="unit_master.php"  method="post"  name="preference_master">
                              ...
                              <span id="prefstatus1234" class="load_error alertsmaster" style="color:#F00" ></span> 
                             	<div class="form_name_cc">Unit<span style="color:#F00">*</span></div>
                               	 ...
                               	 <div class="form_textbox_cc" id="preference_div">
                                     <input type="text" class="form-control preferencename" id="unit1" name="unit1"  placeholder="Unit" tabindex="1" autofocus="autofocus"  data-toggle="tooltip" title="Preference" onChange="valipref()" ></div>
                               </div>
                                  </form> 
                    </div>
		...
		function validate_preferencename()   
			{
				if($("#unit1").val()=="")
				{
					$("#preference_div").addClass("has-error");
					 ...
					 {
						 var a=document.getElementById("unit1").value;
						/*$("#preference_div").removeClass("has-error");
...
{//portionams shtcds
  ...
  if(preference11=="")
  {
	  ...
	  $.ajax({
			...
			data: "value=searchunit&preference="+preference11,
			success: function(msg)
			{
				$('#listall').html(msg);
			}
