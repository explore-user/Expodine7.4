...
else if(($_REQUEST['type']=="tot_sale_online"))
{
    ...
    $string='';
    $string.=" tab_status='closed' AND (tab_mode= 'TA' || tab_mode= 'HD') and tab_complimentary!='Y' AND ";
...
else if(($_REQUEST['type']=="summary_ta"))
{
	...
	$string=" tab_status='closed' AND tab_mode!= 'CS' AND tab_mode = 'TA' AND ";
...
else if(($_REQUEST['type']=="cancel_history_ta"))
{
	...
	$string="";
	$string=" tab_status='Cancelled' AND tab_mode = 'TA' AND ";
        if(isset($_REQUEST['loginstaffsel'])){
            $loginstaffsel = $_REQUEST['loginstaffsel'];
             if($_REQUEST['loginstaffsel']!='null')
                {
                    $string.="tab_cancelledlogin='".$loginstaffsel."' AND ";
...
{
	$string="";
        $string.="tbl_takeaway_billmaster.tab_status='Closed' AND tbl_takeaway_billmaster.tab_mode='TA' AND ";
