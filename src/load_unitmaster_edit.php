...
$_SESSION['u_id']=$_REQUEST['menu'];
$sql_login  =  $database->mysqlQuery("select * from tbl_unit_master  where u_id='".$_SESSION['u_id']."'"); 
$num_cat_s  = $database->mysqlNumRows($sql_login);
if($num_cat_s){
  while($result_cat_s  = $database->mysqlFetchArray($sql_login)) 
	  {
			  $searchname=$result_cat_s['u_name'];
	  }			
} 
else
{
...
//    }     
	 ...
	 ?>
                 <form role="form" action="unit_master.php"  method="post"  name="preference_masteredit">
                            <input type="hidden" name="unitid" id="unitid" class="menuname" style="color:black" value="<?=$result_login['u_id']?>">       
                        	 <div class="first_form_contain">
                              <span id="prefrncstatus1234" class="load_error alertsmaster" style="color:#F00" ></span> 
                             	<div class="form_name_cc">Unit master<span style="color:#F00">*</span></div>
                                
                               	 <div class="form_textbox_cc" id="pref_div1">
                                <input type="text" class="form-control preferencenames" id="addon2" name="unit2"  placeholder="unit2" tabindex="0"  data-toggle="tooltip" title="Preference" value="<?=$result_login['u_name']?>" onchange="valiprefrnc('<?=$result_login['u_id']?>')"></div>
                               </div>
<!--                                        <div class="first_form_contain">
