...
{       $printer_kotip_bill=array();
	...
	{
            $string.=" tab_status='Cancelled' AND ";
			
	}else{
            $string.=" tab_status='closed' and tab_complimentary!='Y' AND ";
        }
        $string.="tab_mode!= 'CS' AND tab_mode='HD' AND ";
		
                 if(isset($_REQUEST['loginstf'])){
            if($_REQUEST['loginstf'] !='null')
            {

                            $string.="tab_cancelledlogin='".$_REQUEST['loginstf']."' AND ";
            }
         }
         if(isset($_REQUEST['staffsel'])){
            if($_REQUEST['staffsel'] !='null')
            {
                            ...
                            $string.="tab_assignedto='".$_REQUEST['staffsel']."' AND ";
            }
         }
				...
				$print .= $center.$bold_on.$from." To ".$to.$bold_off."\n";
                                ...
                                if(isset($_REQUEST['staffsel'])){
                                 if($_REQUEST['staffsel'] !='null'){
                                    ...
                                    $print .= $center.$bold_on."Delivery Staff:".$result_staff['ser_firstname'].$bold_off."\n";
                                }else{
...
$st="Last30days";
	...
	$print='';
        ...
        $print .= $center.$bold_on.'Delivery Report'.$bold_off."\n";
        ...
        $menulist= array(
					new menulist("BillNo", " Staff", " Charge", " Final",$printer_style),
				);
				foreach($menulist as $menulist) {
					$print .=$left.($bold_on.$menulist.$bold_off);
				}
...
$sql_login  =  $database->mysqlQuery("select * from tbl_takeaway_billmaster tbm left join tbl_staffmaster ts on ts.ser_staffid=tbm.tab_assignedto where $string and  tbm.tab_complimentary!='Y'  "); 
...
$num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){$i=0;
		  while($result_login  = $database->mysqlFetchArray($sql_login)) 
			...
			$final=	$final+$result_login['tab_netamt'];
                 ...
                 $menulist= array(
					new menulist($result_login['tab_billno'],$result_login['ser_firstname'], number_format($result_login['tab_delivery_charge'],$_SESSION['be_decimal']),number_format($result_login['tab_netamt'],$_SESSION['be_decimal']),$printer_style),
				);
				foreach($menulist as $menulist) {
					$print .=$left.($bold_on.$menulist.$bold_off);
				}
               ...
               $print .= $left.$vv."\n";
		$menulist= array(
					new menulist("Total","", number_format($del,$_SESSION['be_decimal']),number_format($final,$_SESSION['be_decimal']),$printer_style),
				);
				foreach($menulist as $menulist) {
					$print .=$left.($bold_on.$menulist.$bold_off);
				}		  
	...
	$string_pax="";
	$string_pax="tab_status='Closed' AND tab_mode = 'HD' AND ";
...
//	      echo "select tab_transcbank as bank_name, (sum(tab_transactionamount)) as tot from tbl_takeaway_billmaster tb , tbl_bankmaster b  where  b.bm_id = tb.tab_transcbank and tb.tab_status='Closed' AND $string  group by b.bm_name order by tb.tab_dayclosedate,tb.tab_time ASC";
	  $num_login1   = $database->mysqlNumRows($sql_login1);
		  ...
		  //echo "select $string6_str as tot from tbl_takeaway_billmaster left join tbl_paymentmode on tbl_takeaway_billmaster.tab_paymode=tbl_paymentmode.pym_id where $string6"." $string order by tab_dayclosedate,tab_time ASC";
		  $num_login   = $database->mysqlNumRows($sql_login);
		  if($num_login)
		  {
					...
					if($result_report2['tot'] != "")
			{
					...
					$bilno= array(
						new bilno("Credit",number_format($result_report2['tot'],$_SESSION['be_decimal'] )),
					);
					foreach($bilno as $bilno) {
						$print .=$left.($bilno);
					}
		  ...
		  if($num_login)
		  {
				...
				while($result_report5  = mysqli_fetch_array($sql_login)) 
				{
                  ...
                  ///cpmplimentary-----------
                  $sql_login  =  $database->mysqlQuery("select $string7_str as tot from tbl_takeaway_billmaster where $string7"."$string order by tab_dayclosedate,tab_time ASC"); 
...
//                                            
				...
				$print .= $left.$vv."\n";
				$print .=$center.$bold_on."                         Total : ".number_format($sub_total,$_SESSION['be_decimal']).$bold_off."\n";
				...
				while($result_kots  = mysqli_fetch_array($sql_kotss)) 
						{
                            ...
                            echo "**failed**HD Report Printing Failed: " .$printer_kotname_bill[$key]."( ".$printer_kotip_bill[$key]." )";
                        }
