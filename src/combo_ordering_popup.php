    ...
    $num_combo_pack_details = $database->mysqlNumRows($sql_combo_pack_details);
        if($num_combo_pack_details){$i=0;
            while ($result_combo_pack_details = $database->mysqlFetchArray($sql_combo_pack_details)) {
                $sql_combo_pack_rate =  $database->mysqlQuery("select cpr.cpr_rate  FROM tbl_combo_pack_rates cpr where cpr.cpr_combo_pack_id='".$combo_pack_id."' and cpr.cpr_floor_id='".$_SESSION['floorid']."' and cpr.cpr_mode='DI' "); 
...
?>
<script src="js/combo_ordering.js"></script>
...
<div class="combo-popup" style="height: 550px;">
    ...
    <div class="" style="float: left;width: 100%;height: 5px;line-height: 0px;">
                            <div class="popup_validate" style="width: 100%;height: 5px;line-height: 0px;display:none">
                                  <span class="popup_validate_alert" style="width: 100%; height: 5px; line-height: 0px; ">Select quantity</span>
                            </div> </div>
...
//                                    echo "select mp.mpr_prefeernce,pm.pmr_name,pm.pmr_id FROM tbl_menuprefmaster mp
//                                                                            left join tbl_preferencemaster pm on pm.pmr_id=mp.mpr_prefeernce
//                                                                            left join tbl_combo_pack_menus cpm on cpm.cpm_menu_id=mp.mpr_menuid
//                                                                            where  cpm.cpm_combo_pack_id='".$combo_pack_id."' group by pm.pmr_id";
                                ...
                                <a class="pref_right_btn submit_all" id="combo_add_btn" style="cursor:pointer;margin-top: 8px !important;">ADD</a>
                            </div>
