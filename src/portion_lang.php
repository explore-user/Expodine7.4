...
$database	= new Database();
   ...
   if(isset($_REQUEST['set'])&&($_REQUEST['set']=="cat1")){
    ...
    $querylang=$database->mysqlQuery("select lm_portion_id from tbl_language_portion where lm_portion_id='".$catid1."' and lm_language_id='". $_SESSION['idrr']."'");
    $num_gen9  = mysqli_num_rows($querylang);
		  if($num_gen9)
		  {
    
   
    $querylang1=$database->mysqlQuery("update tbl_language_portion set lm_portion_name='".$cat1."' where lm_portion_id='".$catid1."' and lm_language_id='".$_SESSION['idrr']."'");
        }else
    {
         $insertion['lm_language_id'] =  $_SESSION['idrr'];
         $insertion['lm_portion_name'] = $cat1;
          $insertion['lm_portion_id'] = $catid1;
          $insertid = $database->insert('tbl_language_portion', $insertion);
    }
                       	...
                       	<div id="lista1" class="als-container">
                            <div class="als-viewport" style="width:100% !important">
                                ...
                                <ul class="als-wrapper">
                                    ...
                                    <li class="als-item"><a href="portion_lang.php" class="new_tab_btn active_btn_1">Portion</a></li>
                                    ...
                                    <table class="responstable">  
                                        <thead>
                                         <tr>
                                         	...
                                         	<th>Portion Name-English</th>
                                                <th>Portion Name-<span id="newlang"><?=ucfirst($lang)?> </span></th>
                                        ...
                                        <tbody>
                                           ...
                                           <?php        
                                                     $sql_login1  =  $database->mysqlQuery("select c.pm_id,c.pm_portionname,
(select m.lm_portion_name from tbl_language_portion m  where m.lm_portion_id = c.pm_id and m.lm_language_id ='".$_SESSION['idrr']."' )as name_lang  
from  tbl_portionmaster c order by c.pm_portionname"); 
                                               ...
                                               ?>   
                                                 <tr>
                                                     <td style="min-width:50px;">
                                                         <a href="#" style="display:block" class="edit_list text1" id="edtbtn<?=$result_login1['pm_id']?>" onclick="edit_click('<?=$result_login1['pm_id']?>')"  ><img src="images/edit_page.PNG"></a>
                                                         <a href="#" style="display:none" class="edit_list" id="savebtn<?=$result_login1['pm_id']?>" onclick="save_click('<?=$result_login1['pm_id']?>')"  ><img src="img/save_ico.png"></a>
                                                   </td>

                                                   <td ><?=$result_login1['pm_portionname']?></td>
                                                   
                                               <td ><div style="display:block" id="catnameshow<?=$result_login1['pm_id']?>"><?=$result_login1['name_lang']?></div>
                                                   <div id="catnameedit<?=$result_login1['pm_id']?>" style="display:none"> <input type="text" style="text-align:left;padding-left:10px" class="language_textbx txt1" name="catnamefield" id="catnamefield<?=$result_login1['pm_id']?>"  > </div></td>
                                                 </tr>
                                <?php } } ?>
