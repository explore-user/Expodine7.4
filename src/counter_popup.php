...
$(document).ready(function() {
    ...
    if (!regex.test(key)) {
     ...
     $('.counter_pop_portion_act').find('.counter_portion_view_btn').focus();
   ...
   document.onkeydown= function(ev){
       ...
       if($('.counter_menu_popup').css('display') == 'block'){ 
...
$sql_edit  =  $database->mysqlQuery("select tab_unit_id,tab_base_unit_id,tab_unit_weight, tab_preferencetext,mm.mr_manualrateentry,mm.mr_menuname,td.tab_menuid, td.tab_portion,td.tab_rate,td.tab_qty FROM tbl_takeaway_billdetails td left join tbl_menumaster mm on mm.mr_menuid = td.tab_menuid   where td.tab_billno = '".$_SESSION['cs_order_id']."' and   td.tab_slno='".$slno_edit."' and td.tab_menuid='".$_SESSION['autoloadmenu']."' and tab_bill_addon_slno IS NULL "); 
	...
	if($num_edit)
            ...
            while($result_edit  = $database->mysqlFetchArray($sql_edit)) 
		{
                   ...
                   if($menu_portion!=''){
                   $string.=" and mrc_portion='".$menu_portion."'";
                   }
                   else if($menu_unit_id!=''){
                   $string.=" and mrc_unit_id='".$menu_unit_id."' and mrc_unit_weight='".$menu_base_unit_weight."' ";
                   }
                   else if($menu_base_unit_id!=''){
                   $string.=" and mrc_base_unit_id='".$menu_base_unit_id."'";
                   }
                   else{
                       $string.='';
                   }
            ...
            $sql_addon_edit  =  $database->mysqlQuery("select tbd.tab_slno,tbd.tab_menuid,tbd.tab_portion,tbd.tab_amount,tbd.tab_qty,tbd.tab_rate_type from tbl_takeaway_billdetails tbd where tbd.tab_billno ='". $_SESSION['cs_order_id']."'  and tab_bill_addon_slno='".$slno_edit."' ");
...
<input type="hidden" name="be_search_focus" id="be_search_focus" value="<?=$_SESSION['be_search_focus_counter']?>" />
<?php
$sql_menulist = ("select * from tbl_menumaster as mr LEFT JOIN tbl_menumaincategory as mc ON mr.mr_maincatid=mc.mmy_maincategoryid   WHERE mc.mmy_active='Y'  and mr.mr_active='Y'  and mr.mr_menuid ='".$_SESSION['autoloadmenu']."' order by mr_subcatid");
                                     //echo  "select * from tbl_menumaster as mr LEFT JOIN tbl_menumaincategory as mc ON mr.mr_maincatid=mc.mmy_maincategoryid LEFT JOIN tbl_menusubcategory as ms ON ms.msy_subcategoryid=mr.mr_subcatid LEFT JOIN tbl_menustock ON tbl_menustock.mk_menuid=mr.mr_menuid   WHERE mc.mmy_active='Y' and ms.msy_active ='Y' and mr.mr_active='Y' and  mr.mr_maincatid='" . $categoryid . "' $string  and tbl_menustock.mk_date='" . $_SESSION['date'] . "'  order by mr_subcatid ";
                                    ...
                                    if ($num_menus) {$portion_or_unit='';$packet_or_loose='';
                                        while ($result_menus = $database->mysqlFetchArray($sql_menus)) {
                                           ...
                                           $menu_namecs = $result_menus['mr_menuname'];
                                            $menu_idcs = $result_menus['mr_menuid'];
                                          ...
                                          if($portion_or_unit=='Unit')
                                                {   
                                                    $sql_unit= "select mrc_rate_type,mrc_unit_type from tbl_menurate_counter  WHERE mrc_rate_type='Unit' and mrc_menuid='".$ordered_menuid."'  ";
                                                    ...
                                                    if($num_unit){
                                                        ...
                                                        {
                                                            $packet_or_loose=$result_unit['mrc_unit_type'];
                                                        }
                                                    }
                                                }
                                            ...
                                            $num_arabmenu = $database->mysqlNumRows($sql_arabmenu);
                                            $result_arabmenu = $database->mysqlFetchArray($sql_arabmenu);
                                            $menu_namecs=$result_arabmenu['lm_menu_name'];
                                            // $catid['name'][] = $catname;
                                            //echo $catname;
                                            }
                                            $portion_id=array();
                       ...
                       $menunamelength=  strlen($menu_namecs);
                                if($menunamelength>28){
                               $menunamenew= substr($menu_namecs, 0,30)."...";
                                }else{
                                    $menunamenew=$menu_namecs;
                                }                     
                                            
                                            
	?>  
            <input  type="hidden" id="decimal" value="<?=$_SESSION['be_decimal']?>">
            <input type="hidden" name="idofmenu" id="idofmenu"  value="<?=$menu_idcs ?>"/>
            ...
            <div class="counter_menu_popup_head">
    	<div class="counter_menu_popup_head_text"><?=$menunamenew//$result_menus['mr_menuname']?> </div>
        <div class="counter_menu_popup_head_close"><img src="img/cancel_bill.png"></div>
    </div><!--counter_menu_popup_head-->
    <span style="width:100%;text-align: center;    height: 20px;line-height: 15px;float: left;margin-top: -10px;">
    ...
    <div class="counter_add_popup_contant_cc">
    	<div class="take_away_popup_left_portion_cc" style="position: relative;">
                    <div class="popup_potion_cc">
                            <?php
                          $barcode_in='';  
                          if($bar!=''){
                          $barcode_in=" and mrc_barcode='".$bar."' ";
                          }
                            
                            $menuid=$menu_idcs ;
                            $_SESSION['menu_id']=$menu_idcs ;
                            
                            $sql_menuportion="select * from tbl_menurate_counter left join tbl_portionmaster on mrc_portion=pm_id  where  mrc_menuid='".$menuid."'  $barcode_in  and  mrc_rate_type='".$portion_or_unit."'   $string order by display_order asc";
                            ...
                            {   $rate=0;
                                if($result_portions['mrc_rate_type']=='Portion'){
                                    
                                    $portion_id[]=$result_portions['mrc_portion'];
                                    $menu_rate[]=$result_portions['mrc_rate'];
                                    //$menu_default_portion[]=$result_portions['mta_default'];
                                }
                                else if($result_portions['mrc_rate_type']=='Unit'){
                                    
                                    if($result_portions['mrc_unit_type']=='Loose'){
                                        $base_unit_id[]=$result_portions['mrc_base_unit_id'];
                                        $menu_rate[]=$result_portions['mrc_rate'];
                                        //$menu_default_loose=$result_portions['mta_default'];
                                    }
                                   else if($result_portions['mrc_unit_type']=='Packet'){
                                        $unit_id[]=$result_portions['mrc_unit_id'];
                                        $menu_rate[]=$result_portions['mrc_rate'];
                                        $menu_unit_weight[]=$result_portions['mrc_unit_weight'];
                                 ...
                                 if($_SESSION['incl_bill_format']=='Y'){
                                     ...
                                     $menu_rate_dyn[]=$result_portions['mrc_menu_final_amount'];
                                }
                            ...
                            while ($result_menulistportion = $database->mysqlFetchArray($sql_menulistportion)) {
                                ...
                                ?>
                                <div style="display:block; " title="1" rate="<?=number_format($menu_rate[$p],$_SESSION['be_decimal'])?>"  stock="<?=$stk?>" class="counter_pop_portion  
...
.adon-popup-head3{width:21%;height:auto;float:left;color:#fff;padding-left:2%;font-size:15px;line-height:33px;text-align:center}
...
.adon-popup-contant4{width:21%;height:auto;float:left;color:#fff;padding-left:3%;}
...
.counter_pop_left_portion{height: 260px;}
		  ...
		  while($result_menus1  = $database->mysqlFetchArray($sql_menus1)) 
			  {
				  $manualrateentry=$result_menus1['mr_manualrateentry'];
                                    ...
                                    $sql_menuportion="select * from tbl_menurate_counter where  mrc_menuid='".$menuid."' and  mrc_branchid='".$_SESSION['branchofid']."'";
				    $sql_portions  =  $database->mysqlQuery($sql_menuportion); 
					$num_portions  = $database->mysqlNumRows($sql_portions);
					if($num_portions){$ids="";$v=1;
						while($result_portions  = $database->mysqlFetchArray($sql_portions)) 
							{$rate=0;
								...
								$rate_details=$database->show_portion_ful_details($result_portions['mrc_portion']);
								//load_portionrate
                                                                //$ordered_portionid=trim(json_encode($result_portions['mta_portion']));
                                                                $ordered_portionid=$result_portions['mrc_portion'];
...
//                                                                $resu_portion= json_decode($response_portion['messagesportion'],true);
								...
								$ids="pm_".$result_portions['mrc_portion'];

								
				?>
            
            	<div  style=" <?php if($_SESSION['s_portion_autoday_update']=="Y") {if($weekid==$resu_portion['mrc_portion'] || $result_portions['mrc_portion']==$_SESSION['seperateportion']){ ?> display:block; <?php } else { ?> display:none; <?php }} else { ?> display:block; <?php } ?>" title="<?=$result_portions['mrc_portion']?>" rate="<?=$rate?>" stock="<?=$stk?>" class="take_away_popup_left_portion_contant  
		<?php if($_REQUEST['typesub']=="Edit") { if($result_portions['mrc_portion']==$_REQUEST['portname']){?>add_popup_active_btn <?php }}else{ ?><?php if($v==1){ ?> add_popup_active_btn <?php }} ?>">
				...
				<?php 
				if($_REQUEST['typesub']=="Edit" && ($result_portions['mrc_portion']==$_REQUEST['portname']) )
				{
        ...
        <div class="counter_pop_left_cc" style="margin-left:0.8%">
        	<div class="counter_pop_left_head"><?=$_SESSION['menu_order_popup_qty']?> <?php if($stockon=='Y'){ ?> &nbsp; <span>Left : </span> <span id="stockshowcs" style="color:black;background-color:#ffc877;border-radius:5px;    font-size: 17px;padding: 3px;">  </span>  <?php }?></div>
            <div class="counter_pop_left_portion">
        ...
        <div class="listiing_select_cc">
            <div class="rate_typ_text_cc">  <?php if($manualrateentry=="Y"){ ?> Enter <?php } ?>   <?=$_SESSION['ratename_ta']?> -</div>
             <div class="rate_typ_textox_cc">
                 <input autocomplete="off" type="text" class="rate_typ_textox rateentry <?php if($manualrateentry=="Y"){ ?> border_blink  <?php } ?> " id="mnlentrycs" placeholder="Rate" <?php if($manualrateentry=="N"){  ?> disabled="disabled" <?php } else{?> style="background-color: "   <?php } ?>   onkeypress="return dynamic(event,this.id);" onkeyup="return dynamic(event,this.id);"   manualrate="<?=$manualrateentry?>" value="<?php 
        ...
        <div class="pop_prference_cc" style="position:relative;" >
		  ...
		  if($num_branch)
		  {
			while($result_branch  = mysqli_fetch_array($sql_branch)) 
			{
                                    ...
                                    <select id="insightList" multiple="multiple" class="counter_new_drop_1">
                                     ...
                                     <?php
                                        ...
                                        while ($result_menupref = $database->mysqlFetchArray($sql_pref)) {
                                           ...
                                           $pref_namecs = $result_menupref['pmr_name'];
                                           $pref_idcs = $result_menupref['pmr_id'];
                                    ...
                                    $sql_arabpref=$database->mysqlQuery("SELECT l_pref_name FROM tbl_language_preference left join tbl_languages on ls_id=l_lang_id WHERE l_pref_id='".$result_menupref['pmr_id']."' and l_lang_id='2'");
                                     ...
                                     if($num_arabpref){
                                        while ($result_arabpref = $database->mysqlFetchArray($sql_arabpref)){
                                            ...
                                            $pref_namecs=$result_arabpref['l_pref_name'];
                                   
                                    }}}
                                   $rate_details=$database->show_prefernce_ful_details($pref_idcs);
                   ?>
                                        
                   <option value="<?=$pref_namecs?>" ><?=$pref_namecs?></option>
             ...
             <?php } else { ?>
                 ...
                 <div style="line-height:30px;"  class="counter_new_drop_1"><?=$_SESSION['menu_order_placeholder_nopreference']?></div>
         ...
         padding: 10px;display: flex;flex-direction: column;align-items: flex-end;display: none;border-radius: 4px;overflow-y: auto;height: 112px;">
            ...
            <a href="#" class="counter_pref_right_btn countersale_addnew"><?=$_SESSION['menu_order_popup_submit_button']?></a>
        </div>
        ...
        <?php 
                                 ...
                                 $prf_new_on='';
				if($_REQUEST['typesub']=="Edit" && isset($_REQUEST['prefname']) && $_REQUEST['prefname']!='')
				{ 
                                    $prf_new_on=$_REQUEST['prefname'];
				}?>
        
     <div class="pop_custom_prference_cc">
     <textarea onkeyup="return manual_pref_check();" placeholder="<?=$_SESSION['menu_order_placeholder_edit_manualpref']?>" class="counter_pref_text_area prefrtext pref_key_entry"><?=$prf_new_on?></textarea>
    </div>
    ...
    <?php }} ?>
            ...
            <!-----------------------Add- on Starts------------------------>
                    ...
                    $sql_menuaddon="select mr.mr_menuid, mr.mr_menuname, ma.ma_menuid,mrc.mrc_rate FROM tbl_menu_addons ma, tbl_menurate_counter mrc,tbl_menumaster mr  where ma.ma_menuid='".$_SESSION['autoloadmenu']."' and ma.ma_addon_menuid in (mrc.mrc_menuid) and mr.mr_menuid in (ma.ma_addon_menuid) and mrc.mrc_rate>0 group by ma.ma_addon_menuid";
                    ...
                    <div class="popup-adon-cc" style="display:inline-block">
                        ...
                        <span style="width:100%;text-align: center;    height: 20px;line-height: 15px;float: left;margin-top: -10px;background-color: #000"></span>
                 ...
                 <?php 
                    ...
                    ?>    
                        <div class="adon-popup-contant-row addo_each_menu_div">
                            ...
                            <div class="adon-popup-contant2 addonrate" id="addonrate_<?=$result_menu_addons['mr_menuid']?>"><?=number_format($result_menu_addons['mrc_rate'],$_SESSION['be_decimal'])?></div>
...
<script>
  ...
  $(".manual_pref_qty").on("keyup", function(event){
  ...
  $(document).ready(function(){
     ...
     var show_menu_rate_val = $('#show_menu_rate_val').val();
       if($('#type_action').val()!='Edit'){
            $('#mnlentrycs').val(show_menu_rate_val);//alert("dfgafd");
       }
       ...
       if(dyn=='Y'){
         ...
         $('#mnlentrycs').val(show_menu_rate_val_dyn);
           }, 500);
  ...
  function view_muti_pef(mid){
      ...
      if($('#load_manual_pref').css('display') == 'none')
       ...
       $.post("load_index.php", {set:'load_manual_preference',menu:mid},
               ...
               $.post("load_index.php", {set:'load_manual_preference_edit',menu:mid,mode:'CS'},
					function(data)
					{   
