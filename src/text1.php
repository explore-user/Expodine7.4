...
$printer_style='';
 ...
 $addr=''; $mail_add='';
 $sql_branch =  $database->mysqlQuery("Select be_decimal,be_branchname,be_email_on_dayclose,be_address,be_email,be_phone,be_others1,"
 . "be_others2,be_others3,be_footer1,be_footer2,be_footer3,be_footer4 from tbl_branchmaster where be_branchid='1'"); 
				...
				while($result_branch  = $database->mysqlFetchArray($sql_branch)) 
					{
						$branchname=$result_branch['be_branchname'];
                                                $_SESSION['s_email_on_dayclolse']=$result_branch['be_email_on_dayclose'];
                                                $_SESSION['be_decimal']=$result_branch['be_decimal']; 
 ...
 {	
            ...
            $sql_date=$database->mysqlQuery("Select * From tbl_dayclose order by dc_id desc LIMIT 1");
            ...
            if($num_date){	
                while($result_date  = mysqli_fetch_array($sql_date)){
                        ...
                        $_SESSION['date']=$result_date['dc_day'];
                        $_SESSION['dateopen']=$result_date['dc_dateopen'];
                }
                }	
...
if( $_SESSION['type']=="tot_sales")
{
 ...
 {
	 $_SESSION['fromdt']=$database->convert_date($_SESSION['date']);//$_REQUEST['from'];
 }
 if(isset($_REQUEST['to']))
 {
	 $_SESSION['todt']=$database->convert_date($_SESSION['date']);//$_REQUEST['to'];
 }
  ...
  $subtotalcredit=0;
   ...
   //echo "select $string1_strtacshd as tot from tbl_takeaway_billmaster left join tbl_paymentmode on tbl_takeaway_billmaster.tab_paymode=tbl_paymentmode.pym_id where $string1tacshd"."$stringtacshd order by tab_dayclosedate,tab_time ASC";
	  ...
	  if($num_logincreditta){
          ...
          $stringbnk_dt_di= " bm_dayclosedate between '".$_SESSION['date']."' and '".$_SESSION['date']."' ";  
          
          $stringbnk_dt_ta= " tab_dayclosedate between '".$_SESSION['date']."' and '".$_SESSION['date']."' ";  
	...
	$sql_logincoupon  =  $database->mysqlQuery("select $string3_str as tot from tbl_tablebillmaster left join tbl_paymentmode on tbl_tablebillmaster.bm_paymode=tbl_paymentmode.pym_id where $string3"." $string order by bm_dayclosedate,bm_billtime ASC"); 
	  ...
	  $st="";  $string=""; $final=0;
	  $string.= "(bm.bm_dayclosedate ='".$_SESSION['date']."'  or  tbm.tab_dayclosedate = '".$_SESSION['date']."' ) ";
...
//                                $return.="\r\n";
          ...
          $finaltotal=$totalcash+$totalcredit+$totalcoupon+$totalvoucher+$totalcheque+$totalcp;
			...
			$return.="-----------------------------------------------------------------------";
               ...
               $stringbnk_dt_di_tax= " bm.bm_status='Closed' AND bm.bm_complimentary!='Y' AND bm.bm_dayclosedate between '".$_SESSION['date']."' and '".$_SESSION['date']."' ";  
          
          $stringbnk_dt_ta_tax= " tb.tab_status='Closed' AND tb.tab_complimentary!='Y' AND  tb.tab_dayclosedate between '".$_SESSION['date']."' and '".$_SESSION['date']."' ";                   
          ...
          $return.="\r\n";
				...
				foreach($bilno as $bilno) {
				 ...
				 $return.="\r\n";   
	 			 ...
	 			 $bilno= array(
					new bilno("Total Tax(".$tax_name_val."%)", number_format($all_tax_show,$_SESSION['be_decimal'])),
				);
				foreach($bilno as $bilno) {
					$return .=$left.($bilno);
				}                     
            ...
            if($salesexcltaxcs!=0)
                {   
                 if($_SESSION['uae_tax_enable']=='Y'){
                                  ...
                                  $bilno= array(
					new bilno("Counter Sale Excl.Tax",number_format($uae_subtotal_cs,$_SESSION['be_decimal'])),
				);
				foreach($bilno as $bilno) {
					$return .=$left.($bilno);
				}
                    ...
                    $bilno= array(
				new bilno("Counter Sale Excl.Tax",number_format($salesexcltaxcs,$_SESSION['be_decimal'])),
				);
				foreach($bilno as $bilno) {
					$return .=$left.($bilno);
				}
            ...
            if($salesexcltaxhd!=0)
                {   
                 if($_SESSION['uae_tax_enable']=='Y'){
                                  ...
                                  $bilno= array(
					new bilno("DHome Delivery Excl.Tax",number_format($uae_subtotal_hd,$_SESSION['be_decimal'])),
				);
				foreach($bilno as $bilno) {
					$return .=$left.($bilno);
				}
...
else if($_SESSION['type']=="summary")
{
	...
	/* ******************************************** Category master ******************************************************* */
	  $from=$database->convert_date($_SESSION['date']);//$_REQUEST['from'];


	
	 $to=$database->convert_date($_SESSION['date']);//$_REQUEST['to'];
     ...
     if($_SESSION['fromdt']!="" && $_SESSION['todt']!="")
		{
			...
			$to=$database->convert_date($_SESSION['todt']);
                         ...
                         $string12=$database->convert_date($from);
                         $string13=$database->convert_date($to);
     ...
     if($_SESSION['fromdt']!="" && $_SESSION['todt']!="")
		{
			...
			$string.= " tbm.tab_dayclosedate between '".$from."' and '".$to."' ";
			$reporthead="From ".$database->convert_date($from)."- To ".$database->convert_date($to) ; 
				$string_pax.= "(tbm.tab_dayclosedate  between '".$from."' and '".$to."' ) ";
				
					$strin.=" tbm.tab_dayclosedate between '".$from."' and '".$to."' ";	
		}
		else if($_SESSION['fromdt']!="" && $_SESSION['todt']=="")
		{
			$from=$_SESSION['dateopen'];
  ...
  else if($_SESSION['type']=="regenerate_logs")
{
    	 ...
    	 $reporthead="";
          ...
          $string=""; 
				...
				$to=$_SESSION['dateopen'];
				$string.= " DATE(rt.re_datetime) between '".$from."' and '".$to."' ";
                                
                                
			//$string.= "DATE(rt.re_datetime) ='".$_SESSION['date']."' ";
			$reporthead.="On ".$to ; 
	...
	$return.="\r\n";
	$menulist= array(
                        new regenerate("Bill No","Time","Staff","Reason","First Bill Amount","New BillAmount","Login"),
                );
                    foreach($menulist as $menulist) {
					$return .=$left.($menulist);
			}
         ...
         else if($_SESSION['type']=="consolidated_payment_cr")
{
  ...
  $sql_logincredit  =  $database->mysqlQuery("select distinct (bm_name) as bnk, sum(bm_transactionamount) as tot from tbl_tablebillmaster tb left join tbl_paymentmode on tb.bm_paymode=tbl_paymentmode.pym_id  , tbl_bankmaster b  where  b.bm_id = tb.bm_transcbank and  $stringbnk_dt_di and pym_code='credit' group by bm_name  order by bnk desc "); 
   ...
   $ta_bil_ta_tot=0;
   $sql_login  =  $database->mysqlQuery("Select sum(tab_netamt) as tot_ta  from  tbl_takeaway_billmaster where tab_mode='TA' and tab_status='Closed' and tab_complimentary='N' and tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  $ta_bil_cs_tot=0;
   $sql_login  =  $database->mysqlQuery("Select sum(tab_netamt) as tot_cs from  tbl_takeaway_billmaster where tab_mode='CS' and tab_status='Closed' and tab_complimentary='N' and tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
          ...
          //hd//
                   $ta_bil_hd_tot=0;
   $sql_login  =  $database->mysqlQuery("Select sum(tab_netamt) as tot_hd from  tbl_takeaway_billmaster where tab_mode='HD' and tab_status='Closed' and tab_complimentary='N' and  tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
   ...
   $di_ct_tot=0;
   $sql_login  =  $database->mysqlQuery("Select sum(bm_finaltotal) as di_bill_count_tot from  tbl_tablebillmaster where bm_status='Closed' and bm_complimentary='N' and bm_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  ///ta bills count//
   ...
   $sql_login  =  $database->mysqlQuery("Select count(tab_billno) as ta_bill_countt  from  tbl_takeaway_billmaster where tab_mode='TA' and tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  //cs//
   ...
   $ta_bil_cs=0;
   $sql_login  =  $database->mysqlQuery("Select count(tab_billno) as ta_bill_countc  from  tbl_takeaway_billmaster where tab_mode='CS' and tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
          ...
          //hd//
                   $ta_bil_hd=0;
   $sql_login  =  $database->mysqlQuery("Select count(tab_billno) as ta_bill_counth from  tbl_takeaway_billmaster where tab_mode='HD' and tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  // billcount//
   ...
   $di_ct=0;
   $sql_login  =  $database->mysqlQuery("Select count(bm_billno) as di_bill_count from  tbl_tablebillmaster where bm_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  $ta_hd_cs_ct=0;
   $sql_login  =  $database->mysqlQuery("Select count(tab_billno) as ta_bill_count from  tbl_takeaway_billmaster where tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  $di_ct_bill_cancel=0;
   $sql_login  =  $database->mysqlQuery("Select count(bm_billno) as di_bill_count1 from  tbl_tablebillmaster where bm_status='Cancelled' and bm_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  $ta_hd_cs_ct_bill_cancel=0;
   $sql_login  =  $database->mysqlQuery("Select count(tab_billno) as ta_bill_count1 from  tbl_takeaway_billmaster where tab_status='Cancelled' and  tab_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  //itemcancel//
   $di_ct_item_cancel=0;
   $sql_login  =  $database->mysqlQuery("Select count(ch_kot_cancel_id) as di_bill_count11 from  tbl_tableorder_changes where ch_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
  ...
  $ta_hd_cs_ct_item_cancel=0;
   $sql_login  =  $database->mysqlQuery("Select count(tc_cancel_id) as ta_bill_count11 from  tbl_takeaway_cancel_items where  tc_dayclosedate='".$_SESSION['date']."' "); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
...
$filename = $folder."Reports-".$database->convert_date($_SESSION['date']); 
...
<span style="width:770px;float:left;color:#908e8e;font-size:14px;margin-bottom:0px;margin-top:5px;text-align:left;padding:15px"> 
  <strong style="color:#333;margin-bottom: 4px;float: left; width: 100%;">Dear Customer,</strong><br>
     We are happy to serve you with our automated e-report facility of Daily sales summary of your outlet. 

Please download the attached file along with this email  or view in the browser itself . 
	...
	<div style="width:800px;height:auto;min-height:80px;float:left;background-color:#fff;box-shadow:0px 0px 15px #666;border-bottom:3px solid #ed1e26;">
		 ...
		 <strong style="color:#464646;font-size:20px;">'.$_SESSION['date'].'</strong>

  </div>
		 
         <div style="width:270px;height:auto;float:right;margin:10px 0 0 5px;text-align:right">
         <span  style="color: #666; text-align: center; float: left;width: 100%;padding-top: 20px;font-size: 16px;">Branch</span>
 ...
 <div style="float: left; margin-left: 25px;">
  ...
  $sql_login  =  $database->mysqlQuery("select c.ct_corporatename as company,sum(cd.cd_amount) as amt,count(cd.cd_billno) as ct, "
                ...
                . " cd.cd_dayclosedate='".$_SESSION['date']."' group by company " ); 
 ...
 $msg_temp.= '</table>
  ...
  $sql_login  =  $database->mysqlQuery("select lr.ly_mobileno,lr.ly_firstname, cd.cd_amount from tbl_credit_details cd "
          ...
          . " where cd.cd_settled='N' and ct.ct_credit_type='By Guest' and cd.cd_dayclosedate='".$_SESSION['date']."' " ); 
 ...
 <div style="float: left; margin-left: 25px;">
  ...
  $sql_login  =  $database->mysqlQuery("select sm.ser_firstname as staff ,"
                ...
                . "  ct.ct_credit_type='By Staff' and cd.cd_dayclosedate='".$_SESSION['date']."'  " ); 
...
'; 
        ...
        $mail->Subject = $mail_add." - DAILY SALES SUMMARY REPORT-".$_SESSION['date'];
        $mail->Body = $msg_temp;
...
$nmessage .= "--".$uid."--";
             ...
             $subject = $mail_add." - DAILY SALES SUMMARY REPORT-".$_SESSION['date'];
            //$message = $msg_temp; 

            if(mail($mailto,$subject,$nmessage, $header)) {
                echo "Success";
                $sql_emailsent_updation  =  $database->mysqlQuery("Update tbl_dayclose set dc_dayclose_email_success='Y',dc_last_email_time=NOW() where dc_dateopen='".$_SESSION['date']."'");
    ...
    echo "**failed**Email disabled in General settings**failed**";
   }
