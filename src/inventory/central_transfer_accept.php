...
<select onchange="store_to()"  id="to_store" style=" width: 100px;color:darkred"   >
 ...
 Trn: <?=$_REQUEST['cnt_id']?> | Req: <?=$_REQUEST['req']?>
...
<div class="inv-Password-numbers" style="cursor:pointer">
    ...
    <div class="quick_pop_printer_sec bill_quick_div approve_pop1" style="display:none;">
        <div class="quick_pop_printer"  style="height:185px" >
     <div class="inv-Password">
         <div  onclick="close_pop1()" class="inv-password-img"> <span style="position: absolute;top:5px;left: 120px;" id="load_error11"></span>  <i style="padding: 6px;margin-left: 303px;margin-top: 10px;cursor: pointer;" class="fa fa-close" aria-hidden="true"></i></div>
         <div class="inv-password-msg" style="font-weight:bold;font-size: 15px;margin-top: -33px;"><span>CHOOSE OPTION</span></div>
    <div style="width:100%" class="inv-password-input">

        <select style="width:49%" id="choose_option">
             <option value="">SELECT OPTION</option>
            <option value="accept_partial">Accept Partial Transfer & Stop Sending Balance </option>
             <option value="accept_partial_remain">Accept Partial Transfer & Send Remaining Later</option> 
            <option value="cancel">Cancel Partial Transfer</option> 
        </select>
    ...
    <a style="margin-top: 30px " class="inv-password-btn" href="#" onclick="trasnfer_optional_accept()">SUBMIT</a>
    </div>
...
$(document).on('keyup',function(e) {
    ...
    if(e.which == 9) {
     if ($('#store').is(':focus')) { 
        ...
        $('.inv-req-content').addClass('disablegenerate');
        
        var cnt_id= $("#cnt_id").val();

        var datastring = "set=product_central_transfer_load_live&cnt_id="+cnt_id
                 ...
                 { 
                    ...
                    if(record.tct_edit_value=='null' || record.tct_edit_value=='' || record.tct_edit_value==null){
                     ...
                     var  rate=record.tct_unit_rate;
                      
                     $('#from_branch_name').text(record.tct_from_branch);
                     $('#from_store_name').text(record.tct_from_store);
                  ...
                  $('#from_store').val(record.tct_from_store);
                  $('#to_store').val(record.tct_to_store);
                  $('#from_branch').val(record.tct_from_branch);
   ...
   " <input value='" + unit_type + "' style='width: 8rem;' id='unit_type"+record.tct_id+"' readonly type='text'>"+
    ...
    " <input value='" + ed_qt_wt + "' style='width: 7rem;display:none' id='edit_qty"+record.tct_id+"' onkeyup='check_weight_qty("+record.tct_id+");'  onkeypress='return numdot(this,event);'   type='text'><a title='Qty Update Button' style='margin-left:-25px;color:black;display:none' onclick='edit_weight_qty("+record.tct_id+")'  ><i class='fa fa-check fa-lg' aria-hidden='true' style='color:green;cursor: pointer;border:solid 1px;padding :3px;border-radius:3px;margin-left: 7px;'></i></a> [Cnt Id:" + record.mr_central_id + "]"+
    ...
    $('.inv-req-content').removeClass('disablegenerate');     
...
function numdot(item,evt) {
    ...
    function trasnfer_optional_accept(){
    
    var option= $('#choose_option').val();
    ...
    if(option!=''){
       ...
       var data="set=add_transfer_all_central_accept_live&cnt_id="+cnt_id+"&branch="+branch+"&option="+option;
        ...
        $('#load_error').text('ACCEPTED STOCK TO STORE');
      
        $('#load_error').delay(7000).fadeOut('slow');
       ...
       var data="set=cancel_partial&cnt_id="+cnt_id+"&branch="+branch+"&option="+option;
 ...
 function cancel_req(){
    ...
    if(reason_reject!=''){
       ...
       var data="set=reject_central_transfer_data&cnt_id="+cnt_id+"&branch="+branch+"&reason_reject="+reason_reject;
  ...
  function submit_req(){
      ...
      if( $('#hidden_checker').val()>0){  
        ...
        var data5="set=transfer_partial_check&cnt_id="+cnt_id;
        ...
        var data="set=check_central_id_menu_transfer&cnt_id="+cnt_id;
        ...
        var data="set=add_transfer_all_central_accept_live&cnt_id="+cnt_id+"&branch="+branch;
        ...
        $('#submit_req1').removeClass('disablegenerate'); 
        ...
        $('#load_error').show();
        $('#load_error').css('color','red');
        $('#load_error').text('PARTIAL TRANSFER');
        ...
        $('#load_error').show();
        $('#load_error').css('color','red');
        $('#load_error').text('CHECK INTERNET & SYNC CENTRAL KITCHEN DATA');
        ...
        $('#load_error').delay(1000).fadeOut('slow');
                ...
                $('#load_error').show();
                $('#load_error').css('color','red');
                $('#load_error').text('CHECK BRANCH & TRANSFER ID');
                $('#load_error').delay(1000).fadeOut('slow');
