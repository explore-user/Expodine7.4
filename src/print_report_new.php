...
$branchname='';
$sql_branch =  $database->mysqlQuery("Select be_branchid,be_printall,be_decimal,be_print_on_dayclose,be_branchname,"
        . "be_address,be_email,be_phone,be_others1,be_others2,be_others3,be_footer1,be_footer2,be_footer3,"
        . "be_footer4 from tbl_branchmaster where be_branchid='1'"); 
				...
				while($result_branch  = $database->mysqlFetchArray($sql_branch)) 
					{
						 $branchname=$result_branch['be_branchname'];
                                                 $_SESSION['s_print_on_dayclose']=$result_branch['be_print_on_dayclose'];
                                                 $_SESSION['s_printst']=$result_branch['be_printall'];
                                                 $_SESSION['be_decimal']=$result_branch['be_decimal'];
                                                 $_SESSION['branchofid']=$result_branch['be_branchid'];
					}
		  }
                  
                  
     $sql_table_pt="select * from tbl_printertype WHERE pt_typename='Report Print'";
...
$reports=explode(",",$_REQUEST['type']);
    ...
    // HEADER AND LOGO START
            ...
            if($num_date){	
                while($result_date  = mysqli_fetch_array($sql_date)){
                        ...
                        $date_is_open=$result_date['dc_dateopen'];
 ...
 if($_SESSION['s_print_on_dayclose']=="Y")
{

if($_SESSION['s_printst']=="Y") // printer ye or no
{    
    $sql_kots="Select distinct(pr_printerip),pr_printerport,pr_defaultusb,pr_usbprinterip,pr_usbprinter,pr_style From tbl_printersettings  Where pr_branchid ='1' and pr_printertype='".$_SESSION['reportprint_tp']."'";
...
//                 require_once("printer_functions.php");
		  ...
		  $cur=date("Y-m-d");
		   ...
		   $print .= $center.$bold_on."BRANCH: ".$branchname.$bold_off."\n";
				...
				$sql_kots="Select distinct(pr_printerip),pr_printerport,pr_defaultusb,pr_usbprinterip,pr_usbprinter From tbl_printersettings  Where pr_branchid ='1' and pr_printertype='".$_SESSION['reportprint_tp']."'";
				$sql_kotss  =  mysqli_query($con,$sql_kots); 
				$num_kots  = mysqli_num_rows($sql_kotss);
				if($num_kots){	
...
if($val=="summary_report_cr")
{
  ...
  //echo "select $string1_str as tot from tbl_tablebillmaster left join tbl_paymentmode on tbl_tablebillmaster.bm_paymode=tbl_paymentmode.pym_id where $string1"."$string order by bm_dayclosedate,bm_billtime ASC";
   ...
   //echo "select $string1_strtacshd as tot from tbl_takeaway_billmaster left join tbl_paymentmode on tbl_takeaway_billmaster.tab_paymode=tbl_paymentmode.pym_id where $string1tacshd"."$stringtacshd order by tab_dayclosedate,tab_time ASC";
				...
				if($totalcash!=0)
					...
					foreach($bilno as $bilno) {
						$print .=$left.($bilno);
                             ...
                             $sql_logincredit  =  $database->mysqlQuery("select bm_name as bank_name, (sum(bm_transactionamount)) as tot from tbl_tablebillmaster tb left join tbl_paymentmode on tb.bm_paymode=tbl_paymentmode.pym_id  , tbl_bankmaster b  where b.bm_id = tb.bm_transcbank and $string2 "." $string  group by b.bm_name order by tb.bm_dayclosedate,tb.bm_billtime ASC "); 
                            //echo "select bm_name as bank_name, (sum(bm_transactionamount)) as tot from tbl_tablebillmaster tb left join tbl_paymentmode on tb.bm_paymode=tbl_paymentmode.pym_id  , tbl_bankmaster b  where b.bm_id = tb.bm_transcbank and $string2 "." $string  group by b.bm_name order by tb.bm_dayclosedate,tb.bm_billtime ASC ";
	...
	$totpax_ta_cs_hd=0;                              
        $sql_loginchequeta  =  $database->mysqlQuery("select count(tab_billno) as billno_ta_cs_hd from tbl_takeaway_billmaster  where $stringstacshd"."$stringtacshd "); 
	  $num_loginchequeta   = $database->mysqlNumRows($sql_loginchequeta);
	  if($num_loginchequeta){
		  while($result_loginchequeta  = $database->mysqlFetchArray($sql_loginchequeta)) 
			{ 
	  ...
	  if($num_logintacshd){
       ...
       $tot_excl_in=$tot_payment-$totalcomp;
		  ...
		  $print .= $center.$bold_on."Specified Summary Report ".$bold_off."\n";
				...
				$print .= $center.$bold_on.$date_is_open.$bold_off."\n";
...
else if($val=="consolidated_game_report")
{
	...
	$string.=" DATE(bi_billtime) ='".$date_is_open."' ";
		  ...
		  $print .= $center.$bold_on."Hourly Report".$bold_off."\n";
				...
				$print .= $left.$vv."\n";//ojin
                        ...
                        $sql_hourly_wise =  $database->mysqlQuery("select sum(x.total) as total, x.hour1 as hour1 from( 
                                                    ...
                                                    where bm.bm_dayclosedate='".$date_is_open."' and bm.bm_status='Closed' and bm.bm_complimentary!='Y' group by HOUR(bm.bm_billtime ) union all
                                                    SELECT  SUM(tbm.tab_netamt) as total, HOUR(tbm.tab_time) as hour1  FROM tbl_takeaway_billmaster tbm
                                                    WHERE tbm.tab_dayclosedate='".$date_is_open."' and tbm.tab_status='Closed' and tbm.tab_complimentary!='Y' group by HOUR(tbm.tab_time)
                                                    )x group by  x.hour1");
