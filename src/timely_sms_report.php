...
if(isset($_REQUEST['set_msg'])&& $_REQUEST['set_msg']=="timely_msg" ){
        ...
        if($result==0)
        {
             ...
             $sql_login  =  $database->mysqlQuery("select * from tbl_tablebillmaster where bm_dayclosedate='".$_SESSION['date']."' and  bm_status='Closed' "); 

  
	  $num_login   = $database->mysqlNumRows($sql_login);
	$sql_loginta  =  $database->mysqlQuery("select * from tbl_takeaway_billmaster where tab_dayclosedate='".$_SESSION['date']."' and  tab_status='Closed' "); 
     ...
     $sql_branch1 =  $database->mysqlQuery("Select *  from tbl_timely_sms_entry where tml_time ='".$time_to_send."' and tml_dayclose='".$_SESSION['date']."' and tml_sms='Y'"); 
		  ...
		  {      
             ...
             $sql_sms_updation  =  $database->mysqlQuery("Insert into  tbl_timely_sms_entry(tml_time,tml_hour,tml_dayclose,tml_sms)Values('".$time_to_send."','0','".$_SESSION['date']."','Y')");
...
//                                          
        ...
        $sql_login_fire2  =  $database->mysqlQuery("select tf_active from tbl_firebase_notification_report where tf_report_head='Timely Report' "); 
   ...
   $projectId = 'ed-reports-b5f94'; 
    ...
    'message' => [
       "topic"=> $branch_id_fire,
        'notification' => [
            'title' => $_SESSION['s_branchname']."  - TIMELY REPORT",
            'body' => $body
        ],
        'data' => [
            'key1' => 'value1',
...
//    curl_close($ch);
    ...
    //var_dump($result);
		...
		fclose($fp);
                ...
                $locationsms='..\util\Timely_sms/';
                
                 if(!is_dir($locationsms)){
                     
            mkdir($locationsms , 0777,TRUE);   
                 }
          if(!file_exists("..\util\Timely_sms/".$_SESSION['date'].".txt")) 
         {
          $myfile = fopen("..\util\Timely_sms/".$_SESSION['date'].".txt", "w") or die("Unable to open file!");
