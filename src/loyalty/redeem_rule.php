...
if(isset($_REQUEST['set_delete_rules_master'])&&($_REQUEST['set_delete_rules_master']=="delete_rules_master")){
    ...
    $database->mysqlQuery("DELETE FROM tbl_loyalty_redeem_rule WHERE lyr_id = '" .$mst_id_del."'");
...
if(isset($_REQUEST['set_update_master1'])&&($_REQUEST['set_update_master1']=="update_master1")){
             ...
             $mid1=$_REQUEST['up_id'];
               ...
               $sql_smsattemt_updation  =  $database->mysqlQuery("UPDATE tbl_loyalty_redeem_rule SET lyr_point='".$point1."',lyr_amount='".$amt1."' where lyr_id='".$mid1."'");
...
if(isset($_REQUEST['set_add_master'])&&($_REQUEST['set_add_master']=="add_master")){
        ...
        $insertion['lyr_point'] = mysqli_real_escape_string($database->DatabaseLink,trim($point)); 
        
        $insertion['lyr_amount']= mysqli_real_escape_string($database->DatabaseLink,trim($amt)); 
        
	$insertid      =  $database->insert('tbl_loyalty_redeem_rule',$insertion); 
...
if(isset($_REQUEST['set_edit_master']) && $_REQUEST['set_edit_master']=="edit_rules_master"){
  ...
  $loy_qry1_upd = $database->mysqlQuery("select * from tbl_loyalty_redeem_rule where lyr_id='".$_REQUEST['edit_master_id']."' ");
    $num_loy1_upd = $database->mysqlNumRows($loy_qry1_upd);
     if($num_loy1_upd)
     {
         while($loyalty_listing_edit = $database->mysqlFetchArray($loy_qry1_upd))
         {
             ...
             $pnt=  $loyalty_listing_edit['lyr_point'];
             $amt=  $loyalty_listing_edit['lyr_amount'];
...
<head>
    ...
    <body class="fixed-left">
            ...
            <div class="content-page">
                ...
                <div class="content">
                    ...
                    <div class="page_head_detl_sec">
                        ...
                        <a href="point_rule.php"><div style="width:120px;background-color: darkred;margin-right: 10px;float:right"  class="top-head-add-btn"> Point Rule</div></a>
     ...
     $loy_qry_rules = $database->mysqlQuery("select * from tbl_loyalty_redeem_rule");
     ...
     if($num_loy_rules)
     {
         while($loyalty_listing_rules = $database->mysqlFetchArray($loy_qry_rules))
         {
                      ...
                      $mst_id=$loyalty_listing_rules['lyr_id'];
                                ?>
                                <tr>
                                    <td id="redeem_one" > <?=$loyalty_listing_rules['lyr_point']?></td> 
                                    
                                    <td><?=$loyalty_listing_rules['lyr_amount']?></td>  
                                    <td>
                                    	<a data-toggle="modal" data-target="#earing-point-master-add" onclick="return edit_master('<?=$loyalty_listing_rules['lyr_id']?>');"  href="#" class="action-btn" ><span class="ti-pencil"></span></a>
                                        <a href="#" class="action-btn" onclick="return delete_rules('<?=$loyalty_listing_rules['lyr_id']?>');" ><span class="ti-trash"></span></a>
  ...
  function edit_master(em){
    ...
    function submit_rules_master(){
       ...
       if(sts!="update"){
           ...
           var redeem_one=$('#redeem_one').text();
        if(redeem_one==""){
      ...
      alert("Redeem Rule Already Added");
      location.reload();
    }
    }else{
