...
$_SESSION['pagid']=6;
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['preference1']))
{
	
	$insertion['pmr_name'] 		=  mysqli_real_escape_string($database->DatabaseLink,$_REQUEST['preference1']);

    $sql=$database->check_duplicate_entry('tbl_preferencemaster',$insertion);
	 if($sql!=1)
	{
	$insertid              			=  $database->insert('tbl_preferencemaster',$insertion);
	$database->updateexpodine_machines(); 
        //add xml code starts
	  $lastid='';
	 $sql_login  =  $database->mysqlQuery("select pmr_id from tbl_preferencemaster where 	pmr_name='".$insertion['pmr_name']."'"); 
	  $num_login   = $database->mysqlNumRows($sql_login);
		  while($result_login  = $database->mysqlFetchArray($sql_login)) 
			{
				$lastid=$result_login['pmr_id'];
			}
...
//header("location: preference_master.php?msg=2");
    ...
    {    
        ...
        echo 'window.location.href="preference_master.php?msg=2";';
        echo '</script>';
        echo '<noscript>';
        echo '<meta http-equiv="refresh" content="0;url=preference_master.php?msg=2" />';
        echo '</noscript>'; exit;
    }
}
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['preference12']))
{
	$id=$_REQUEST['prefrncid'];
	$preference12=$_REQUEST['preference12'];
	
$query3=$database->mysqlQuery("update tbl_preferencemaster set pmr_name='$preference12' where pmr_id='$id'");
$database->updateexpodine_machines(); 
...
//header("location: preference_master.php?msg=3");
    ...
    {    
        ...
        {  
        echo '<script type="text/javascript">';
        echo 'window.location.href="preference_master.php?msg=3";';
        echo '</script>';
        echo '<noscript>';
        echo '<meta http-equiv="refresh" content="0;url=preference_master.php?msg=3" />';
        echo '</noscript>'; exit;
    }
}
$alert="";
...
<div class="mian">
	<div class="view-container">
		<div style=" top: 58px;"  id="container">
			<div class="breadcrumbs">
				<ul>
					...
					<li><a style="cursor:pointer">Preference</a></li>
				...
				<div class="als-viewport" style="width:100% !important">
                   ...
                   <div style="background:#fff; width:100%; height:auto; margin:0 0 5px 1px;padding-right:4px; border:1px #B6B6B6  solid;float:left">
                           ...
                           <div class="col-sm-2" style="padding-right: 5px;padding-left: 0px;margin-bottom:5px;width:25%">
                           		<span class="filte_new_text">Preference</span>
                                <input type="text" class="form-control filte_new_box" id="pref" name="pref" placeholder="Preference" onKeyUp="validateSearch()">
                            </div>
                            ...
                            <div class="col-sm-2 nopadding" style="width: 24.666667% !important;">
                                ...
                                <div style="margin-left:2%;width: 47%;" class="search_btn_member_invoice filte_new_box_btn"><a href="preference_master.php" >Reset</a></div>
                            </div>
                        </div><!--form_group-->
                   ...
                   <div class="col-md-12 add_btn_cc_2">
                      <div class="btn_cc_2">
                   		<a tittle="Add" href="#" id="add_pref" class="md-trigger add_btn_2" data-modal="modal-17" onClick="testpref()" ></a>
                      </div>  
                   </div>
                   <div class="col-md-12 contant_table_cc">
                        <table class="table_report scroll tablesorter"  width="100%" border="0" cellspacing="5" id="listall">
                            <thead>
                              <tr>
                                <th width="50%" height="35px">Preference</th>
                                 <td width="50%">Action</td>
                              </tr>
                             </thead>
                                 <?php
	 $sql_login  =  $database->mysqlQuery("select * from tbl_preferencemaster"); 
	  $num_login   = $database->mysqlNumRows($sql_login);
	  if($num_login){
		  while($result_login  = $database->mysqlFetchArray($sql_login)) 
			{
	 ...
	 ?>
    						<tr id="ids_<?=$result_login['pmr_id']?>"  class="select">
                                <td><?=$result_login['pmr_name']?></td>
                                
                             
                                <td>
                                 <a href="#" class="md-trigger_prfrnc" id="ids_<?=$result_login['pmr_id']?>" ><img src="images/edit_page.PNG"></a>
                                <input type="hidden" name="hiddenmenuid" id="hiddenmenuid" value="<?=$result_login['pmr_id']?>">
			...
			<div class="md-content">
				...
				<div>
                    <div class="col-lg-12 col-md-12 no-padding">
                              <form role="form" action="preference_master.php"  method="post"  name="preference_master">
                              ...
                              <span id="prefstatus1234" class="load_error alertsmaster" style="color:#F00" ></span> 
                             	<div class="form_name_cc">Preference<span style="color:#F00">*</span></div>
                               	 ...
                               	 <div class="form_textbox_cc" id="preference_div">
                                     <input type="text" class="form-control preferencename" id="preference1" name="preference1"  placeholder="Preference" tabindex="1" autofocus="autofocus"  data-toggle="tooltip" title="Preference" onChange="valipref()" ></div>
                               </div>
                                  </form> 
                    </div>
		...
		function validate_preferencename()   
			{
				...
				{
					$("#preference_div").addClass("has-error");
						  document.preference_master.preference1.focus();
					 ...
					 {
						 var a=document.getElementById("preference1").value;
						/*$("#preference_div").removeClass("has-error");
...
function delete_confirm(id)
{
	...
	if(check==true)
	{
		window.location="preference_master.php?id="+id+"&delete=yes";
	}
}	
</script>
<script type="text/javascript">
...
{//portionams shtcds
  ...
  if(preference11=="")
  {
	  ...
	  $.ajax({
			...
			data: "value=searchpreference&preference="+preference11,
			success: function(msg)
			{
				$('#listall').html(msg);
			}
