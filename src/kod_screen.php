...
if($_SESSION['s_kod_takeaway']=='Y' && $_SESSION['s_kod_dinein']=='Y') {
	...
	header("location:kod_screen_two.php");
}
}
$set='';
...
{
	$set="ta";
         $modeswitchbutton="Dine In";
         $modeswitchid="dine";
         
}
if(($_SESSION['s_kod_dinein']=='Y')&&($mode==""||$mode=="dine"))
{       $modeswitchbutton="Take Away";
        $modeswitchid="ta";
	$set="dine";
}
...
/***************************************  autorefresh starts ******************************************************************  */
 ...
 $('#columns').load('load_kod_screen.php?value=loadkodscreen&set='+setnm+'&counter='+seltd+'&numberofkot='+numItems);
 $('#kod_top_notification').load('load_kod_notification_refresh.php?mode='+mode+'&processcount='+processcount+'&kotcounters='+seltd);
 $('#kod_top_notification').load();
	}, 1000); 
 ...
 var url_check=$('#url_check').val();
   ...
   var new_id=url_check.split('title_value=');
   
 var mode=$('#mode').val();
   
   if(new_id[1]!=''){
       
   $('.load_cl'+new_id[1]).addClass('table_floor_select_btn_act');
 
 
    if(new_id[1]!='ALL' && new_id[1]=='')
    
    $('.all_load').removeClass('table_floor_select_btn_act');
...
<div class="middle_container">
    <div style="width:100%" class="top_site_map_cc ">
        ...
        <div class="table_floor_select_btn" title="<?=$modeswitchbutton?>"><a href="kod_screen_two.php" >ALL</a></div>
        <div class="table_floor_select_btn" title="<?=$modeswitchbutton?>"><a href="kod_screen.php?mode=<?=$modeswitchid?>"><?=$modeswitchbutton?></a></div>
...
<?php
      ...
      <div class="top_counter_sel_contain">
            	<div class="kod_split_head take_away_txt_clr">
                    ...
                    <?php if(($_SESSION['s_kod_takeaway']=='Y')&&($mode==""||$mode=="ta")){ ?>Take Away <?php }else
			if($_SESSION['s_kod_dinein']=='Y'&&($mode==""||$mode=="dine")){ ?>Dine In <?php }?>
                   <!-- <span class="kod_sub_counter_list">( <?=$_SESSION['kotcounterselected_name']?> )</span>-->
                    </div>
                </div>
                <div class="counter_select_contain" style="width:87%;">
                	<div class="multi_selector_kod">
                        <div style="width:100%;" class="kod_multi_selectbox">
                           ...
                           <div class="table_floor_select_btn swichkitchen all_load table_floor_select_btn_act" title="ALL"><a href="#" >ALL</a></div>
                            
                            <?php   $sql_login  =  $database->mysqlQuery("select * from tbl_kotcountermaster LEFT JOIN tbl_branchmaster ON "
                                    . " tbl_kotcountermaster.kr_branchid=tbl_branchmaster.be_branchid where tbl_kotcountermaster "); 
											  $num_login   = $database->mysqlNumRows($sql_login);
											  if($num_login){
												  while($result_login  = $database->mysqlFetchArray($sql_login)) 
													{ ?>
                           ...
                           <div class="table_floor_select_btn swichkitchen load_cl<?=$result_login['kr_kotcode']?>" title="<?=$result_login['kr_kotcode']?>"><a href="#" ><?=$result_login['kr_kotname']?></a></div>
            ...
            <?php 
            $sql_kodscreen  =  $database->mysqlQuery("select be_kod_screen from tbl_branchmaster"); 
            $num_kodscreen  = $database->mysqlNumRows($sql_kodscreen);
            if($num_kodscreen)
            {
              $result_kodscreen  = $database->mysqlFetchArray($sql_kodscreen);
           
                if($result_kodscreen['be_kod_screen']=='Option_2')
              {
                  $screenclass='class="zoom_1"';
              }
              else if($result_kodscreen['be_kod_screen']=='Option_3')
              {
                  $screenclass='class="zoom_2"';
              }
              else
              {
                  $screenclass="";
...
<div id="columns" <?=$screenclass?>>
  <input type="hidden" name="countrseltd" id="countrseltd" value="<?=$_SESSION['kotcounterselected']?>">          
			 ...
			 if($value=="ALL")
			 {
				   ...
				   $sql_menulist= "Select distinct(bd.tab_kotno_new) as kotno,bm.tab_billno as blno,mn.mr_time_min as menutime,bm.tab_time as biltime,km.kr_kotcode as kotcod From tbl_takeaway_billmaster as bm LEFT JOIN tbl_takeaway_billdetails as bd ON bd.tab_billno=bm.tab_billno LEFT JOIN tbl_menumaster as mn ON  mn.mr_menuid=bd.tab_menuid LEFT JOIN tbl_kotcountermaster as km ON km.kr_kotcode=mn.mr_kotcounter  Where bm.tab_dayclosedate ='".$_SESSION['date']."' And bm.tab_kotno != '' And (bm.tab_status='Generated' OR bm.tab_status='Ready' OR bm.tab_status='Processing') AND  (bd.tab_status='Generated' OR bd.tab_status='Ready' OR bd.tab_status='Processing') and mn.mr_show_in_kod='Y' group by bd.tab_kotno_new,kotcod order by bd.tab_kotno_new DESC ";
                                   ...
                                   }else  if(($_SESSION['s_kod_dinein']=='Y'&&($mode==""||$mode=="dine")))
				   {
			 ...
			 }else
			 {
				   if(($_SESSION['s_kod_takeaway']=='Y')&&($mode==""||$mode=="ta"))
				   {//bm.tab_customermobile as mobile,bm.tab_customername,bm.tab_hdcustomerid ,bm.tab_status as status,bm.tab_mode,km.kr_kotname,
				   $sql_menulist= "Select distinct(bd.tab_kotno_new) as kotno,bm.tab_billno as blno,mn.mr_time_min as menutime,bm.tab_time as biltime,km.kr_kotcode as kotcod From tbl_takeaway_billmaster as bm LEFT JOIN tbl_takeaway_billdetails as bd ON bd.tab_billno=bm.tab_billno LEFT JOIN tbl_menumaster as mn ON  mn.mr_menuid=bd.tab_menuid LEFT JOIN tbl_kotcountermaster as km ON km.kr_kotcode=mn.mr_kotcounter  Where bm.tab_dayclosedate ='".$_SESSION['date']."' And bm.tab_kotno != '' And (bd.tab_status='KOT_Generated' OR bd.tab_status='Ready' OR bd.tab_status='Processing') AND km.kr_kotcode='".$value."'   AND  (bd.tab_status='Generated' OR bd.tab_status='Ready' OR bd.tab_status='Processing') and mn.mr_show_in_kod='Y' group by bd.tab_kotno_new,kotcod   order by bd.tab_kotno_new DESC ";
				   }else  if(($_SESSION['s_kod_dinein']=='Y'&&($mode==""||$mode=="dine")))
				   {
  ...
  <div class="pin" <?php if($dishtime<$current_time){ ?> style="background-color: #ff8181"<?php }else{  }?> onClick="return popupfunction('<?=$result_menus['kotno']?>','<?=$result_menus['blno']?>','<?=$_SESSION['date']?>','<?=$_SESSION['s_kod_takeaway']?>','<?=$mode?>','<?=$_SESSION['s_kod_dinein']?>','<?=$result_menus['kotcod']?>')">
                  ...
                  <div class="kod_detail_number" <?php if($a=='b'){ ?> style="background-color:red"<?php }?>>
                   ...
                   <?php if($tbl==""){?><div class="kod_table_number_steward"><span></span><span></span></div><?php } else{?>
                      <div class="kod_table_number_steward"><?php if($result_menus['tbl']!=""){?><span>Table: <?=$result_menus['tbl']?>(<?=$result_menus['prefix']?>)</span> | <span>Steward: <?=$staff?></span><?php }?></div><?php }?>
                      
                   <?php
				   $sql_menulist1='';
			if(($_SESSION['s_kod_takeaway']=='Y')&&($mode==""||$mode=="ta"))
			 {
				     $sql_menulist1= "Select tbl_takeaway_billdetails.tab_count_combo_ordering,tbl_takeaway_billdetails.tab_unit_weight,tbl_takeaway_billdetails.tab_rate_type,tbl_takeaway_billdetails.tab_unit_type,um.u_name,bum.bu_name,sum(tbl_takeaway_billdetails.tab_qty) as qty,mi.mes_imagethumb as image,tbl_takeaway_billdetails.tab_preferencetext as pref,tbl_takeaway_billdetails.tab_slno,tbl_menumaster.mr_menuname as menuname,tbl_menumaster.mr_menuid,tbl_portionmaster.pm_portionshortcode as portion,tbl_takeaway_billdetails.tab_status  as status  From tbl_menumaster left Join tbl_takeaway_billdetails On tbl_takeaway_billdetails.tab_menuid = tbl_menumaster.mr_menuid left Join tbl_portionmaster On tbl_portionmaster.pm_id =tbl_takeaway_billdetails.tab_portion left join tbl_unit_master um on um.u_id=tbl_takeaway_billdetails.tab_unit_id left join tbl_base_unit_master bum on bum.bu_id=tbl_takeaway_billdetails.tab_base_unit_id LEFT JOIN tbl_menuimages mi on mi.mes_menuid=tbl_menumaster.mr_menuid Where tbl_takeaway_billdetails.tab_kotno_new = '".$result_menus['kotno']."' AND (tbl_takeaway_billdetails.tab_status='KOT_Generated' OR tbl_takeaway_billdetails.tab_status='Ready' OR tbl_takeaway_billdetails.tab_status='Processing')  AND   tbl_menumaster.mr_kotcounter= '".$result_menus['kotcod']."' and tbl_menumaster.mr_show_in_kod='Y' group by tbl_takeaway_billdetails.tab_menuid,tbl_takeaway_billdetails.tab_portion,tbl_takeaway_billdetails.tab_unit_id,tbl_takeaway_billdetails.tab_base_unit_id,tbl_takeaway_billdetails.tab_unit_weight  order by tab_slno ASC ";
                                     //echo "Select tbl_takeaway_billdetails.tab_unit_weight,tbl_takeaway_billdetails.tab_rate_type,tbl_takeaway_billdetails.tab_unit_type,um.u_name,bum.bu_name,tbl_takeaway_billdetails.tab_qty as qty,mi.mes_imagethumb as image,tbl_takeaway_billdetails.tab_preferencetext as pref,tbl_takeaway_billdetails.tab_slno,tbl_menumaster.mr_menuname as menuname,tbl_menumaster.mr_menuid,tbl_portionmaster.pm_portionshortcode as portion,tbl_takeaway_billdetails.tab_status  as status  From tbl_menumaster left Join tbl_takeaway_billdetails On tbl_takeaway_billdetails.tab_menuid = tbl_menumaster.mr_menuid left Join tbl_portionmaster On tbl_portionmaster.pm_id =tbl_takeaway_billdetails.tab_portion left join tbl_unit_master um on um.u_id=tbl_takeaway_billdetails.tab_unit_id left join tbl_base_unit_master bum on bum.bu_id=tbl_takeaway_billdetails.tab_base_unit_id LEFT JOIN tbl_menuimages mi on mi.mes_menuid=tbl_menumaster.mr_menuid Where tbl_takeaway_billdetails.tab_billno = '".$result_menus['blno']."' AND (tbl_takeaway_billdetails.tab_status='KOT_Generated' OR tbl_takeaway_billdetails.tab_status='Ready' OR tbl_takeaway_billdetails.tab_status='Processing')  AND   tbl_menumaster.mr_kotcounter= '".$result_menus['kotcod']."' and tbl_menumaster.mr_show_in_kod='Y'  order by tab_slno DESC ";
                                     
                         }else if(($_SESSION['s_kod_dinein']=='Y'&&($mode==""||$mode=="dine")))
			 {
				  $sql_menulist1=" select tor.ter_count_combo_ordering,tor.ter_unit_weight,tor.ter_rate_type,tor.ter_unit_type,um.u_name,bum.bu_name, mi.mes_imagethumb as image,tor.ter_orderno,tor.ter_slno as slno,sum(tor.ter_qty) as qty,tor.ter_preference as per1,tor.ter_portion,tor.ter_preferencetext as per2,tmr.mr_menuname as menuname,tmr.mr_menuid,tmr.mr_time_min as menutime,tpr.pm_portionshortcode as portion,tpr.pm_portionname,tor.ter_status as status from tbl_tableorder as tor LEFT JOIN tbl_menumaster as tmr ON tor.ter_menuid=tmr.mr_menuid LEFT JOIN  tbl_portionmaster as tpr ON tpr.pm_id=tor.ter_portion left join tbl_unit_master um on um.u_id=tor.ter_unit_id left join tbl_base_unit_master bum on bum.bu_id=tor.ter_base_unit_id LEFT JOIN tbl_menuimages mi on mi.mes_menuid=tmr.mr_menuid   where  tor.ter_kotno='".$result_menus['kotno']."' and tor.ter_dayclosedate='".$_SESSION['date']."'  AND   tmr.mr_kotcounter= '".$result_menus['kotcod']."' and tmr.mr_show_in_kod='Y' group by tor.ter_menuid,tor.ter_portion,ter_unit_id,ter_base_unit_id,ter_unit_weight  order by ter_slno,tor.ter_count_combo_ordering ASC ";
			 
                                 // echo "select tor.ter_unit_weight,tor.ter_rate_type,tor.ter_unit_type,um.u_name,bum.bu_name,mi.mes_imagethumb as image,tor.ter_orderno,tor.ter_slno as slno,tor.ter_qty as qty,tor.ter_preference as per1,tor.ter_portion,tor.ter_preferencetext as per2,tmr.mr_menuname as menuname,tmr.mr_menuid,tmr.mr_time_min as menutime,tpr.pm_portionshortcode as portion,tor.ter_status as status from tbl_tableorder as tor LEFT JOIN tbl_menumaster as tmr ON tor.ter_menuid=tmr.mr_menuid LEFT JOIN  tbl_portionmaster as tpr ON tpr.pm_id=tor.ter_portion left join tbl_unit_master um on um.u_id=tor.ter_unit_id left join tbl_base_unit_master bum on bum.bu_id=tor.ter_base_unit_id LEFT JOIN tbl_menuimages mi on mi.mes_menuid=tmr.mr_menuid   where  tor.ter_kotno='".$result_menus['kotno']."' and tor.ter_dayclosedate='".$_SESSION['date']."'  AND   tmr.mr_kotcounter= '".$result_menus['kotcod']."' and tmr.mr_show_in_kod='Y' order by ter_slno DESC ";
                         }
                         
	$sql_menus1  =  $database->mysqlQuery($sql_menulist1); 
	$num_menus1  = $database->mysqlNumRows($sql_menus1);
	if($num_menus1){$i=1;$pref='';$unit='';$combo_ordering_count=array();
                        $combo_pack_rate=0;
                        $combo_menu_qty=0;
                        $combo_qty=0;
			...
			//echo "select (SUM(CI.tc_cancel_qty)+'".$result_menus1['qty']."') AS oldqty  FROM tbl_takeaway_cancel_items CI where CI.tc_billno ='".$result_menus['blno']."' and CI.tc_bill_slno='".$result_menus1['tab_slno']."'  GROUP BY CI.tc_billno, CI.tc_bill_slno";
			   ...
			   {     
                              ...
                              //////////////////// COMBO NAMES /////////////////////////////////// 
                               ...
                               $sql_menulist2="SELECT (sum(ch_cancelled_qty)+'".$result_menus1['qty']."') as oldqty,ch_combo_pack_cancelled_qty as combo_cancelled,MAX(ch_entrydate) as lastcanceltime from tbl_tableorder_changes WHERE ch_orderno='".$result_menus1['ter_orderno']."' and ch_kotno='".$result_menus['kotno']."' and ch_orderslno='".$result_menus1['slno']."' group  by ch_orderno,ch_orderslno";
			//echo "SELECT (sum(ch_cancelled_qty)+'".$result_menus1['qty']."') as oldqty,MAX(ch_entrydate) as lastcanceltime from tbl_tableorder_changes WHERE ch_orderno='".$result_menus1['ter_orderno']."' and ch_kotno='".$result_menus['kotno']."' and ch_orderslno='".$result_menus1['slno']."' group by ch_orderno,ch_orderslno";
                               $sql_menus2  =  $database->mysqlQuery($sql_menulist2); 
                                $num_menus2  = $database->mysqlNumRows($sql_menus2);
                                if($num_menus2){
                                    $a='b';
                                    ...
                                    $result_menus2  = $database->mysqlFetchArray($sql_menus2);
                                    $_SESSION['lastcanceltime']=$result_menus2['lastcanceltime'];
                                }
...
//                                                                $resu= json_decode($response['messages'],true);
                    ...
                    <div class="kod_list_item">
                       ...
                       if($combo_ordering_count_each && !in_array($combo_ordering_count_each,$combo_ordering_count)){
                        ...
                        ?>
                       <p class="combo_sec_kod"><?=$combo_name?></p>
                       <?php }else{
                           $combo_name='';
                       } ?>
                       <h1><?php if($a=='b'){?><strike style="color:red"><?php }?><span class="quantity123"><?php if($combo_qty>0){ if($a=='a'){ echo $combo_qty.'*'; echo $combo_menu_qty;} else { echo $combo_qty+$result_menus2['combo_cancelled'].'*'; echo $combo_menu_qty;}} else{ if($a=='a'){ echo $result_menus1['qty'];} else { echo $result_menus2['oldqty'];} }?></span> <span><?php if($result_menus1['portion']!=''){ echo '('.$result_menus1['portion'].')';} else { echo '('.$unit.')'; }?></span> * <span class="kod_list_item123" id="<?=$result_menus1['mr_menuid']?>"><?=$menu_name?></span><?php if($a=='b'){?></strike><?php }?>
                     	<div class="kod_order_status">
                        <?php  if(($_SESSION['s_kod_takeaway']=='Y')&&($mode==""||$mode=="ta")) {?>
			<?php if($result_menus1['status']=='Ready'){ ?><img src="img/served-icon-1.png"> <?php } if($result_menus1['status']=='KOT_Generated'){ ?><img src="img/pending-icon.png"> <?php } if($result_menus1['status']=='Processing'){ ?><img src="img/process-ico.png"> <?php } ?>
                        <?php }else if(($_SESSION['s_kod_dinein']=='Y'&&($mode==""||$mode=="dine"))) { ?>
 ...
 <div class="kod_item_details_popup_contant">
   ...
   <span class="kod_popup_table_head">
       	<table width="100%" border="0" cellspacing="0">
                 <tbody><tr>
                   <th width="10%">Sl</th>
                   <th width="61%">Item Name <input class="chekbx" type="checkbox" id="chekbx"></th>
                   <th width="15%">Qty</th>
                 </tr>
            </tbody></table> 
    </span> 
            ...
            <span class="kod_popup_table">
                ...
                <?php if(( $_SESSION['s_kod_dinein']=='Y')&&($mode==""||$mode=="dine")){ ?>
                <a href="#"><div class="kod_pop_bottom_srv_btn served">Served</div></a><?php }
                else if(($_SESSION['s_kod_takeaway']=='Y')&&($mode==""||$mode=="ta")) { ?>
                <a href="#"><div class="kod_pop_bottom_srv_btn served">Packed</div></a> <?php } ?>
