...
DAY AND CURDATE( )";
	...
	$cur=date("Y-m-d");
	  ...
	  }else
	  {
        ...
        {	
	 $date=date("Ymd"); 
	$string=" tbm.tab_mode='HD' AND tbm.tab_status='Closed' AND ";
...
}else if($_REQUEST['type']=="order_hd")
{
 ...
 $date=date("Ymd");
$string="";
$string.="tbl_takeaway_billmaster.tab_status='Closed' AND tbl_takeaway_billmaster.tab_mode='".$_REQUEST['typesale']."' AND ";
        if($_REQUEST['staffsel']!="null"){
            $string.=" tbl_takeaway_billmaster.tab_assignedto = '".$_REQUEST['staffsel']."' AND ";
        }
if($_REQUEST['hidfr']!="" && $_REQUEST['hidto']!="")
		{
			$from=$database->convert_date($_REQUEST['hidfr']);
