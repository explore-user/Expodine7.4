			...
			for($fl=1;$fl<=$fieldnumbr;$fl++)
						...
						?>
                         ...
                         <?php if(isset($sheet['cells'][$x][1])){ 
                                ...
                                <div class="search_name_box_main filter_date_textbox_cc">
                            <div class="text-selection_name filter_date_textbox_name"><?=isset($sheet['cells'][$x][2]) ? $sheet['cells'][$x][2] : ''?>:</div>
                            <?php $x++;} ?>
                            <?php if(isset($sheet['cells'][$x][2])){
                             ...
                             <?php if(($sheet['cells'][$x][1])=="Field ".$fl." : Id"){$fieldid=$sheet['cells'][$x][2];$x++;  ?>
                                 <select  class="form-control1 filter_by_date_textbox_cc_selt" id="<?=$fieldid ?>">
                             ...
                             <?php }else if(($sheet['cells'][$x][2])=="TextBox") {$x++; ?>
                                ...
                                <?php if(($sheet['cells'][$x][1])=="Field ".$fl." : Id"){$fieldid=$sheet['cells'][$x][2];$x++; ?>
                                 ...
                                 <?php if(($sheet['cells'][$x][2])=="Date"){$x++; ?>
                                     <input type="text" class="form-control1" id="<?=$fieldid ?>" >
                                      <script>
									   $(document).ready(function() {
										$("#<?=$fieldid?>").datepicker({
											changeMonth: true,
...
//										  $("#<?=$fieldid?>").change(function (){
	  ...
	  <a href="#" class="reportsubmit" id="<?=$_REQUEST['reportid']?>"><div class="filter_sub_btn_cc">Submit</div></a>
		<?php }
	}?>
<!--	<a href="#" class="reportsubmit" id="<?=$fieldid ?>"><div class="filter_sub_btn_cc">Submit</div></a>-->
     <?php   

	/**********************************************************field define ends*****************************************************/

} if($_REQUEST['checkvalue']=='loadreportdata'){ 
session_start();
error_reporting(0);
$localhost=mysqli_connect($_SESSION['host'], $_SESSION['user'], $_SESSION['pas'],$_SESSION['db']);
$template='';
