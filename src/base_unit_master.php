...
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['baseunit1']))
{
	
	$insertion['bu_name'] 		=  mysqli_real_escape_string($database->DatabaseLink,$_REQUEST['baseunit1']);

    $sql=$database->check_duplicate_entry('tbl_base_unit_master',$insertion);
	 if($sql!=1)
	{
	$insertid              			=  $database->insert('tbl_base_unit_master',$insertion);
	$database->updateexpodine_machines(); 
	 ...
	 if (!headers_sent())
    {    
        header('Location: base_unit_master.php?msg=2');
        exit;
        }
    else
        {  
        echo '<script type="text/javascript">';
        echo 'window.location.href="base_unit_master.php?msg=2";';
        echo '</script>';
        echo '<noscript>';
        echo '<meta http-equiv="refresh" content="0;url=base_unit_master.php?msg=2" />';
        echo '</noscript>'; exit;
    }
}
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['baseunit2']))
{
	$id=$_REQUEST['baseunitid'];
	$unit2=$_REQUEST['baseunit2'];
...
$query3=$database->mysqlQuery("update tbl_base_unit_master set bu_name='$unit2' where bu_id='$id'");
$database->updateexpodine_machines(); 

	 if (!headers_sent())
    {    
        header('Location: base_unit_master.php?msg=3');
        exit;
        }
    else
        {  
        echo '<script type="text/javascript">';
        echo 'window.location.href="base_unit_master.php?msg=3";';
        echo '</script>';
        echo '<noscript>';
        echo '<meta http-equiv="refresh" content="0;url=base_unit_master.php?msg=3" />';
        echo '</noscript>'; exit;
    }
}
$alert="";
				...
				<div class="als-viewport" style="width:100% !important">
                   ...
                   <div style="background:#fff; width:100%; height:auto; margin:0 0 5px 1px;padding-right:4px; border:1px #B6B6B6  solid;float:left">
                           ...
                           <div class="col-sm-2" style="padding-right: 5px;padding-left: 0px;margin-bottom:5px;width:25%">
                           		<span class="filte_new_text">Base Unit Name</span>
                                <input type="text" class="form-control filte_new_box" id="baseunit" name="baseunit" placeholder="Base Unit Name" onKeyUp="validateSearch()">
                            </div>
...
<!--                                <div style="margin-left:2%; width: 47%;" class="search_btn_member_invoice filte_new_box_btn"><a href="#" onClick="validateSearch();">Search</a></div>-->
                                <div style="margin-left:2%;width: 47%;" class="search_btn_member_invoice filte_new_box_btn"><a href="base_unit_master.php" >Reset</a></div>
                            </div>
                        </div><!--form_group-->
                        ...
                        <?php  if(in_array("unit_master", $_SESSION['menusubarray'])) { ?> 
                                  <div class="col-sm-2 nopadding" style="width: 24.666667% !important;">
                                      ...
                                      <div style="margin-left:2%;width: 47%;" class="search_btn_member_invoice filte_new_box_btn"><a style="    width: 154px;background-color: #3b6470" href="unit_master.php" >UNIT MASTER</a></div>
                   ...
                   <div class="col-md-12 contant_table_cc">
                        <table class="table_report scroll tablesorter"  width="100%" border="0" cellspacing="5" id="listall">
                            <thead>
                              <tr>
                                  <td width="9%">Sl No</td>
                                <th width="40%" height="35px">Base Unit</th>
                                 ...
                                 <?php
	 $sql_login  =  $database->mysqlQuery("select * from tbl_base_unit_master"); 
...
//                      }
	 ...
	 ?>
    						<tr id="ids_<?=$result_login['bu_id']?>"  class="select">
                                                     <td width="10%"><?=$i?></td>
                                <td><?=$result_login['bu_name']?></td>
                                  
                                
                             
                                <td>
                                    <a style="pointer-events: none " href="#" class="md-trigger_prfrnc" id="ids_<?=$result_login['bu_id']?>" ><img src="images/edit_page.PNG"></a>
                                <input type="hidden" name="hiddenmenuid" id="hiddenmenuid" value="<?=$result_login['bu_id']?>">
			...
			<div class="md-content">
				...
				<div>
                    <div class="col-lg-12 col-md-12 no-padding">
                        <form role="form" action="base_unit_master.php"  method="post"  name="preference_master">
                        	 <div class="first_form_contain">
                              <span id="prefstatus1234" class="load_error alertsmaster" style="color:#F00" ></span> 
                             	<div class="form_name_cc">Base Unit<span style="color:#F00">*</span></div>
                                
                               	 <div class="form_textbox_cc" id="preference_div">
                                     <input type="text" class="form-control preferencename" id="baseunit1" name="baseunit1"  placeholder="base Unit" tabindex="1" autofocus="autofocus"  data-toggle="tooltip" title="Preference" onChange="valipref()" ></div>
                               </div>
                                  </form> 
                    </div>
...
function valipref()
{
	 var a=$("#baseunit1").val().trim();
	
	  $.ajax({
			type: "POST",
			url: "load_divcheckmenu.php",
			data: "value=checkprefrnc22&mid="+a,
		...
		function validate_preferencename()   
			{
				...
				{
					$("#preference_div").addClass("has-error");
						  document.preference_master.baseunit1.focus();
                                                  alert("Enter Base unit");
						  return false;
				}
                                
                                var alphanumers = /^[a-zA-Z0-9 ]+$/;
                                if(!alphanumers.test($("#baseunit1").val())){
                              $("#preference_div").addClass("has-error");
                               document.preference_master.baseunit1.focus();
					 ...
					 {
						 var a=document.getElementById("baseunit1").value;
						/*$("#preference_div").removeClass("has-error");
...
function delete_confirm(id)
{
	...
	if(check==true)
	{
		window.location="unit_master.php?id="+id+"&delete=yes";
	}
}	
</script>
<script type="text/javascript">
function testpref()
{
		document.getElementById('baseunit1').value = '';
	$("#prefstatus1234").text("");
...
{//portionams shtcds
  ...
  if(preference11=="")
  {
	  ...
	  $.ajax({
			...
			data: "value=searchbaseunit&preference="+preference11,
			success: function(msg)
			{
				$('#listall').html(msg);
			}
