...
$database	= new Database();
   ...
   if(isset($_REQUEST['set'])&&($_REQUEST['set']=="cat1")){
    ...
    $querylang=$database->mysqlQuery("select f_floor_id from tbl_language_floor where f_floor_id='".$catid1."' and f_lang_id='". $_SESSION['idrr']."'");
    $num_gen9  = mysqli_num_rows($querylang);
		  if($num_gen9)
		  {
    
   
    $querylang1=$database->mysqlQuery("update tbl_language_floor set f_floor_name='".$cat1."' where f_floor_id='".$catid1."' and f_lang_id='".$_SESSION['idrr']."'");
        }else
    {
         $insertion['f_lang_id'] =  $_SESSION['idrr'];
         $insertion['f_floor_name'] = $cat1;
          $insertion['f_floor_id'] = $catid1;
          $insertid = $database->insert('tbl_language_floor', $insertion);
    }
                       	...
                       	<div id="lista1" class="als-container">
                            <div class="als-viewport" style="width:100% !important">
                                ...
                                <ul class="als-wrapper">
                                    ...
                                    <li class="als-item"><a href="floor_lang_change.php" class="new_tab_btn active_btn_1">Floor</a></li>
                                    ...
                                    <table class="responstable">  
                                        <thead>
                                         <tr>
                                         	...
                                         	<th>Floor Name-English</th>
                                                <th>Floor Name-<span id="newlang"><?=ucfirst($lang)?> </span></th>
                                        ...
                                        <tbody>
                                           ...
                                           <?php        
                                                     $sql_login1  =  $database->mysqlQuery("select c.fr_floorid,c.fr_floorname,
(select m.f_floor_name from tbl_language_floor m  where m.f_floor_id = c.fr_floorid and m.f_lang_id ='".$_SESSION['idrr']."' )as name_lang  
from  tbl_floormaster c order by c.fr_floorname"); 
                                               ...
                                               ?>   
                                                 <tr>
                                                     <td style="min-width:50px;">
                                                         <a href="#" style="display:block" class="edit_list text1" id="edtbtn<?=$result_login1['fr_floorid']?>" onclick="edit_click('<?=$result_login1['fr_floorid']?>')"  ><img src="images/edit_page.PNG"></a>
                                                         <a href="#" style="display:none" class="edit_list" id="savebtn<?=$result_login1['fr_floorid']?>" onclick="save_click('<?=$result_login1['fr_floorid']?>')"  ><img src="img/save_ico.png"></a>
                                                   </td>

                                                   <td ><?=$result_login1['fr_floorname']?></td>
                                                   
                                               <td ><div style="display:block" id="catnameshow<?=$result_login1['fr_floorid']?>"><?=$result_login1['name_lang']?></div>
                                                   <div id="catnameedit<?=$result_login1['fr_floorid']?>" style="display:none"> <input type="text" style="text-align:left;padding-left:10px" class="language_textbx txt1" name="catnamefield" id="catnamefield<?=$result_login1['fr_floorid']?>"  > </div></td>
                                                 </tr>
                                <?php } } ?>
